<?php
class Minv_fg_in extends CI_Model {
    ...
    function read($param){
        switch ($param["method"]) {
            case "read_group_conveyor":
                ...
                break;
            case "SP_INV_FG_IN":
                return $this->SP_INV_FG_IN($param);
                break;
            case "read_in_fg":
    ...
    function SP_INV_FG_IN($param){
        $this->load->database();
        $SQL_CALLSP = "EXEC SP_INV_FG_IN
        @VUSERNAME=?,
        @VIN_DATE=?,
        @VMODULE=?,
        @VDATA=?
    ...
    function list_poly_detail($param){
        ...
        $this->db->order_by( "A.PART_NO", "ASC");

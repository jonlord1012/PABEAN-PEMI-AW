<?php
class Mbcin_271_aw extends CI_Model
{
    ...
    function read_data($param)
    {
        ...
        $this->db->where(
            ...
            C.SERI = 3 AND 
            B.NOMORDOKUMEN IN (
                SELECT DISTINCT INVOICE_NO  FROM [dbo].[AW_DRAFT_TO_PLB]
            )
            "
        );
        $arr_field = array(
    ...
    function read_dokumen_barang_tarif($param)
    {
        $this->load->database();
        $this->db->distinct();
        #$Tsql = "SELECT distinct * FROM _FROM_PORTAL40_BAHANBAKU WHERE [NOMOR AJU] = '" . $param['NOMOR_AJU'] . "' AND [SERI BARANG] = '" . $param['SERI_BARANG'] . "'";
        #$this->db->query($Tsql);
        $this->db->select(" *");
        $this->db->from("VW_FROM_PORTAL_BAHANBAKU ");
        $this->db->where("NOMORAJU = '" . $param['NOMOR_AJU'] . "' ");

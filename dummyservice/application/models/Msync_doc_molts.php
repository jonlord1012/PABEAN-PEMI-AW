...
class Msync_doc_molts extends CI_Model {
    ...
    function read($param){
        switch ($param["method"]) {
            ...
            case "read_draft_bc":
                return $this->read_draft_bc($param);
                break;
            case "read_data":
            ...
            case "read_draft_supplier":
                ...
                break;
            case "read_invoice_list":
                return $this->read_invoice_list($param);
                break;
    ...
    function read_draft_bc($param){
        $this->load->database();
        $SQL = "
            SELECT DISTINCT MODE_CODE,MODE_NAME,MODE_SHORT FROM a_matrix 
            WHERE
            MODE_CATEGORY='DRAFT_DOKUMEN_MOLTS' AND 
    ...
    function read_original($param){
        ...
        $this->db->select("
        INVOICE_NO,
        FORMAT(LAST_UPDATE,'yyyy-MM-dd') as LAST_UPDATE,
        FORWARDER,
        CONSIGNEE,
        BUYER,
        SHIPPING_MODE,
        FORMAT(ETD,'yyyy-MM-dd') as ETD,
        FORMAT(ETA,'yyyy-MM-dd') as ETA,
        PAYMENT,
        ORDER_NO,
        ITEM_CODE,
        ITEM_TYPE1,
    ...
    function sync_nomoraju($param){
        $this->load->database();
        $SQL_CALLSP = "EXEC SP_SYNC_AJU_MOLTS
        @VIDCOMPANY=?,
        @VUSERNAME=?
        ";
        $data = array(
    ...
    function sync_automapping_supplier($param){
        ...
        $this->db->simple_query("
                ...
                upload_molts_detail
                SET 
                upload_molts_detail.MAPP_SUPPLIER = B.MAPP_SUPPLIER
                FROM 
                upload_molts_detail A
                LEFT JOIN 
                (
                    SELECT SHIPPER,MAPP_SUPPLIER FROM upload_molts_detail A
                    WHERE
                    A.SHIPPER IN (
                    SELECT 
                    A.SHIPPER
                        FROM upload_molts_detail A
                    WHERE
                    A.MAPP_SUPPLIER is NULL OR
                    A.MAPP_SUPPLIER =''
                    GROUP BY
    ...
    function sync_manualmapping_supplier($param){
        $this->load->database();
        $this->db->simple_query("
                update upload_molts_detail set
                mapp_supplier='".$param['KODE_INTERNAL']."'
                where
                SHIPPER='".$param['SHIPPER']."' AND 
    ...
    function sync_manualmapping_itempart($param){
        ...
        $this->db->simple_query("
                ...
                where
                DESCRIPTION1='".$param['DESCRIPTION1']."' AND 
    ...
    function sync_automapping_itempart ($param){
        ...
        $this->db->simple_query("
                ...
                upload_molts_detail
                SET 
                upload_molts_detail.MAPP_PARTNO = B.MAPP_PARTNO
                FROM 
                upload_molts_detail A
                LEFT JOIN 
                (
                    SELECT A.DESCRIPTION1,A.MAPP_PARTNO FROM upload_molts_detail A
                                        WHERE A.DESCRIPTION1 IN (
                                        SELECT 
                                        A.DESCRIPTION1
                                        FROM upload_molts_detail A
                                        WHERE 
                                        A.MAPP_PARTNO ='' OR A.MAPP_PARTNO is NULL
                                        GROUP BY A.DESCRIPTION1, A.MAPP_PARTNO ) AND
                                        A.MAPP_PARTNO<>'' AND
                                        A.MAPP_PARTNO IS NOT NULL 
                                        GROUP BY
                                        A.DESCRIPTION1,A.MAPP_PARTNO
                ) B
    ...
    function read_dokumenpabean_header($param){
        ...
        *
        ...
        $this->db->where("
        A.NOMOR_AJU= (select TOP 1 NOMOR_AJU from upload_molts_detail where INVOICE_NO='".$param['INVOICE_NO']."')
        ");
    ...
    function read_dokumenpabean_detail($param){
        ...
        A.ID_HEADER = (
            ...
            where 
            NOMOR_AJU IN (
                select A.NOMOR_AJU from upload_molts_detail A where A.INVOICE_NO='".$param['INVOICE_NO']."' group by A.NOMOR_AJU 
            )
            group by 
            ID_HEADER_ORI
        )
    ...
    function read_draft_supplier ($param){
        ...
        $this->db->distinct();
        $this->db->select("
        A.MAPP_SUPPLIER,B.NAMA,B.ALAMAT,B.KODE_NEGARA,C.URAIAN_NEGARA,B.NPWP
        ");
        $this->db->from("upload_molts_detail A");
        $this->db->join("referensi_pemasok B","A.MAPP_SUPPLIER = B.KODE_INTERNAL","left");
        $this->db->join("referensi_negara C","B.KODE_NEGARA = C.KODE_NEGARA","left");
        $this->db->where("
            A.BC_TYPE is NULL AND 
    ...
    function read_invoice_list($param){
        $this->load->database();
        $varlike=" ";
        ...
        $arr_field = array(
            ...
            'A.INVOICE_DATE'=>'INVOICE_DATE',
            'C.CURRENCY'=>'CURRENCY',
            'A.INVOICE_TERM'=>'INVOICE_TERM',
            'A.SHIPPING_MODE'=>'SHIPPING_MODE'
        );
        ...
        $query = $this->db->query("
            SELECT TOP 10 A.INVOICE_NO,A.INVOICE_DATE,A.CURRENCY,A.INVOICE_TERM,A.INVOICE_TERM,A.SHIPPING_MODE
            FROM (
            SELECT
            DISTINCT A.INVOICE_NO,FORMAT(A.INVOICE_DATE,'yyyy-MM-dd') as INVOICE_DATE,A.CURRENCY,A.INVOICE_TERM,A.SHIPPING_MODE
            FROM upload_molts_detail A
            WHERE
            A.BC_TYPE is NULL AND 
    ...
    function SP_MOLTS_DRAFT_SELECT_INVOICE($param){
        $this->load->database();
        $SQL_CALLSP = "EXEC SP_MOLTS_DRAFT_SELECT_INVOICE
        @VIDCOMPANY=?,
        @VUSERNAME=?,
        @VINVOICE=?,
        @VVENDOR=?
    ...
    function proses_save_dokumen_draft_molts($param){
        $this->load->database();
        $SQL_CALLSP = "EXEC SP_MOLTS_CREATE_DOKUMEN_DRAFT
        @VIDCOMPANY=?,
        @VUSERNAME=?,
        @VHEADER=?,
        @VDOKUMEN=?,

<?php
class Mmst_pen_teknisi extends CI_Model {
    ...
    function create($param){
      $vdata = json_decode($param['data'],true);
        $field = array(
            'TEKNISI_NAME' => $vdata['TEKNISI_NAME'],
            'TEKNISI_DESCRIPTION' => $vdata['TEKNISI_DESCRIPTION'],
            'TEKNISI_CODE' => $vdata['TEKNISI_CODE'],
            'ID_COMPANY' => $param['ID_COMPANY'],
            'CREATE_USER'=>$param['VUSERNAME']
        );
        $this->load->database();
        $this->db->set('CREATE_DATE', 'getdate()', false);
        if (!$this->db->insert('mst_teknisi', array_filter($field)))
        {   
            $out= array(
                'success'=>false,
    ...
    function update($param){
        ...
        if(!$this->db->update('mst_teknisi', $field )){
            $out= array(
                'success'=>false,
    ...
    function edit($param){
        $vdata = $param['data'];
        $this->load->database();
        $query = $this->db->get_where('mst_teknisi',array(
            'ID'=>$vdata['ID']
        ));

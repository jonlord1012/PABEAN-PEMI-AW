<?php
class Msync_doc extends CI_Model {
    ...
    function read_receivingdata_header($param){
        ...
        $this->db->where("
        A.RECEIPT_NO IN (select B.RECEIPT_NO from wh_inv_detail B where B.INVOICE_NO = '".$param['INVOICE_NO']."' group by B.RECEIPT_NO )
        ");
    ...
    function read_invoice_mapping_itempart($param){
        $this->load->database();
        $this->db->select("
        DISTINCT A.INVOICE_NO,A.VENDOR,A.ITEM_NUMBER,A.DESCRIPTION
        ",false);
        $this->db->from("upload_".$param['module'].'_detail A');
        $this->db->where("
    ...
    function sync_automapping_itempart ($param){
        ...
        $this->db->simple_query("
                ...
                upload_coo_detail
                SET 
                upload_coo_detail.MAPP_PARTNO = B.PART_NO
                FROM 
                upload_coo_detail A
                LEFT JOIN mst_part B on A.ITEM_NUMBER = B.PART_NO 
                WHERE 

Ext.define("TDK.INVENTORY.inv_subassy_aw_out.Cinv_subassy_aw_out", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cinv_subassy_aw_out",
  init: function (view) {
    this.control({
      "inv_subassy_aw_out button[pid=btrefresh_main]": { click: this.btrefresh_main_click },
      "inv_subassy_aw_out button[pid=btreceiving_integrasi]": { click: this.btreceiving_integrasi_click },
  ...
  btrefresh_main_click: function () {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("inv_subassy_aw_out GRIDinv_subassy_aw_out grid[pid=GRIDinv_subassy_aw_out]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btreceiving_integrasi_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMinv_subassy_aw_auto", "TDK.INVENTORY.inv_subassy_aw_out.FRMinv_subassy_aw_auto", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
      ...
      Ext.MessageBox.confirm(
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(vconfig.service_api + "inv_subassy_aw_out/inv_subassy_aw_out", params, "POST", me.var_global.jwt);

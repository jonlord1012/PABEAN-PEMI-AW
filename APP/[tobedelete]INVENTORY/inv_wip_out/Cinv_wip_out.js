Ext.define("TDK.INVENTORY.inv_wip_out.Cinv_wip_out", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cinv_wip_out",
  init: function (view) {
    this.control({
      "inv_wip_out button[pid=btprocess_sync]": { click: this.btprocess_sync_click },
      "inv_wip_out button[pid=btrefresh_main]": { click: this.btrefresh_main_click_with_cbo },

      "FRMsync_doublecheck button[pid=btrefresh]": { click: this.btrefresh_click },
      "FRMsync_doublecheck datefield[name=production_date]": { change: this.btrefresh_click },

      "FRMwip_out_scrapitem button[pid=out_toscrap_btselectpart]": { click: this.out_toscrap_btselectpart_click },
      "FRMwip_out_scrapitem button[pid=btitem_save]": { click: this.out_scrapitem_btitem_save_click },
    });
    this.listen({
      store: {},
    });
  ...
  btrefresh_main_click_with_cbo: function () {
    try {
      ...
      var GRID = Ext.ComponentQuery.query("inv_wip_out GRIDinv_wip_out grid")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btprocess_sync_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMsync_doublecheck", "TDK.INVENTORY.inv_wip_out.FRMinv_wip_out_sync_doublecheck", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btproses_sync_doublecheck_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.ComponentQuery.query("FRMsync_doublecheck")[0];
      var grid_doublecheck = Ext.ComponentQuery.query("FRMsync_doublecheck grid[pid=GRIDsync_doublecheck]")[0];
      ...
      var params = Ext.encode({
        method: "PROSES_SP_FG_GET_DOUBLECHECK",
        proddate: moment(vproddate.getValue()).format("YYYY-MM-DD"),
        module: "proses",
      });

      var hasil = COMP.run.getservice(vconfig.service_api + "inv_wip_out/inv_wip_out", params, "POST", localStorage.getItem("ST_NJC_JWT"));
  ...
  btrefresh_click: function () {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("FRMsync_doublecheck grid[pid=GRIDsync_doublecheck]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("wipout_dokumen_detail", "TDK.INVENTORY.inv_wip_out.dokumen_detail.dokumen_detail", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  out_to_memo_scrap_click: function () {
    try {
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMwip_out_scrapitem", "TDK.INVENTORY.inv_wip_out.FRMwip_out_scrapitem", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  out_toscrap_btselectpart_click: function () {
    try {
      var FRM = Ext.ComponentQuery.query("FRMwip_out_scrapitem form")[0];
      var GRIDitem = Ext.ComponentQuery.query("FRMwip_out_scrapitem grid")[0];
      ...
      var popup = Ext.define("TDK.INVENTORY.inv_wip_out.popup_itempart", {
  ...
  out_scrapitem_btitem_save_click: function () {
    try {
      var me = this;
      var GRIDmanual = Ext.ComponentQuery.query("FRMwip_out_scrapitem grid[pid=GRIDout_toscrap_mt]")[0];
      var popup = Ext.ComponentQuery.query("FRMwip_out_scrapitem")[0];
      var GRIDMAIN = Ext.ComponentQuery.query("inv_wip_out GRIDinv_wip_out grid[pid=GRIDinv_wip_out]")[0];
      ...
      Ext.MessageBox.confirm(
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(vconfig.service_api + "inv_wip_out/inv_wip_out", params, "POST", me.var_global.jwt);
            hasil.then(function (content) {
              var val = Ext.decode(content, true);

Ext.define("TDK.INVENTORY.inv_wnt_out.Cinv_wnt_out", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cinv_wnt_out",
  init: function (view) {
    this.control({
      /*"btn add new input"*/
      "inv_wnt_out GRIDinv_wnt_out button[pid=btnew_inv_wnt_out]": { click: this.btnew_input_click},
      /* main grid :: add new supply out */
      "inv_wnt_out GRIDinv_wnt_out button[pid=GRIDinv_wnt_out_btnew]": { click: this.btnew_click },
      /* main grid :: history */
      "inv_wnt_out grid[pid=GRIDinv_wnt_out]": { itemdblclick: this.GRIDinv_wnt_out_itemdblclick },

      /* frm input :: save button */
      "FRMinv_wnt_out button[pid=FRMinv_wnt_out_btsave]": { click: this.btsave_click },

      /* search button */
      "FRMinv_wnt_out button[pid=btsearch_technician]": { click: this.btsearch_technician_click },
      "FRMinv_wnt_out button[pid=btsearch_process_name]": { click: this.btsearch_process_name_click },
      "FRMinv_wnt_out button[pid=btsearch_carline]": { click: this.btsearch_carline_click },
      "FRMinv_wnt_out button[pid=btsearch_doc]": { click: this.btsearch_doc_click },

      /* add item button */
      "FRMinv_wnt_out button[pid=FRMinv_wnt_out_btitem_part]": { click: this.btitem_part_click_ },
    });
    this.listen({
      store: {},
    });
  ...
  btnew_input_click:function() {
    try{
      ...
      COMP.run.getmodulepopup("FRMinv_wnt_out", "TDK.INVENTORY.inv_wnt_out.FRMinv_wnt_out", mainpanel);
    } catch(ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  btsearch_doc_click:function(btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      console.log("btsearch_doc_click Button klicked");
      return this.popup_search(btn);
      //COMP.run.getmodulepopup("FRMinv_wnt_out", "TDK.INVENTORY.inv_wnt_out.FRMinv_wnt_out", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("TDK.INVENTORY.inv_wnt_out.popup_search_", {
        ...
        items: [
          {
            xtype: "grid",
            pid: "GRIDFRMinv_wnt_out_popupsearch",
  ...
  setproperty: function (modulename) {
    try {
      ...
      switch (modulename) {
        ...
        case "inv_out_doc":
          ncol.push(
            { header: "KODE JENIS DOKUMEN", dataIndex: "KJD", sortable: true, width: 130, filter: { xtype: "textfield" } },
            { header: "NOMOR DOKUMEN", dataIndex: "NO_DOK", sortable: true, width: 200, filter: { xtype: "textfield" } },
          );
          break;
        default:
          break;
  ...
  btremove_part_click: function (ctor, rows) {
    
    try {

      var me=this,arr=[]; 
    
      var remove_grid = ctor.store.removeAt(rows);

      var grid_list = (ctor.getStore().getData().items);
        
      Ext.each(grid_list, function(items) {
        arr.push(items.data)
      }, this);

      var rec = new Ext.data.Store({ data:arr });
      
      ctor.grid.reconfigure(rec);
  ...
  btitem_part_click_: function (btn) {
    try {
      ...
      var popup = Ext.define("TDK.INVENTORY.inv_wnt_out.popup_list_item", {
        extend: "Ext.window.Window",
        ...
        items: [
          {
            ...
            columns: [
              ...
              { sortable: true, width: 150, filter: { xtype: "textfield" }, header: "TANGGAL AJU", dataIndex: "NOMOR_AJU" },
              { sortable: true, width: 150, filter: { xtype: "textfield" }, header: "BC TYPE", dataIndex: "BC_TYPE" },
  ...
  fieldto_grid:function(rec) {

    var me=this,arr=[];
    var GRIDFRM = Ext.ComponentQuery.query('FRMinv_wnt_out grid[pid=GRIDFRMinv_wnt_out]')[0];
    var popup = Ext.ComponentQuery.query('popup_list_item')[0];

    try {

      var grid_list = (GRIDFRM.getStore().getData().items);
      ...
      arr.push({
        PART_NO:rec.data.PARTNO,
        PART_NAME:rec.data.PART_NAME,
        KODE_SATUAN:rec.data.PART_UOM,
        QTY_REQUESTED:rec.data.QTY_AVAILABLE,
        QTY_SUPPLIED:'1',
        INFO:''
      });

      var rec = Ext.data.Store({ data:arr });

      GRIDFRM.reconfigure(rec);
      GRIDFRM.getStore().sync()
    } catch(ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
    popup.close()

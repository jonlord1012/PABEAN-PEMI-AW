Ext.define("NJC.EXIM.bcin_40.Cbcin_40", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbcin_40",
  init: function (view) {
    this.control({
      "bcin_40 button[pid=btrefresh]": { click: this.btrefresh_click },
      "bcin_40 combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      // "FRMbcin_40 button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
      // "FRMbcin_40 button[pid=btsearch]": { click: this.btsearch_click },
    });
    this.listen({
      store: {},
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cbcin_40");
    } catch (ex) {
  ...
  btrefresh_click: function () {
    try {
      var GRID = Ext.ComponentQuery.query("bcin_40 GRIDbcin_40 grid[pid=GRIDbcin_40]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcin_40", "NJC.EXIM.bcin_40.FRMbcin_40", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  */
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      ...
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.ComponentQuery.query("FRMbcin_40")[0];
      if (!popup) {
        popup = Ext.create("NJC.EXIM.bcin_40.FRMbcin_40");
        popup.on(
          "show",
          function () {
            try {
              ...
              var hasil = COMP.run.getservice(vconfig.service_api + "bcin_40/bcin_40", params, "POST", me.var_global.jwt);
              hasil.then(function (content) {
                ...
                if (val.success == "true") {
                  ...
                  if (val.B_DOKUMEN !== null) {
                    ...
                    var GRIDbcin_40_dokumen = popup.query("grid[pid=GRIDbcin_40_dokumen]")[0];
                    Ext.Array.each(B_DOKUMEN, function (rec) {
                      GRIDbcin_40_dokumen.getStore().add(rec);
                    });
                  }
                  ...
                  if (val.B_PENGANGKUT !== null) {
                    ...
                    var GRIDbcin_40_pengangkut = popup.query("grid[pid=GRIDbcin_40_pengangkut]")[0];
                    Ext.Array.each(B_PENGANGKUT, function (rec) {
                      GRIDbcin_40_pengangkut.getStore().add(rec);
                    });
                  }
                  if (val.B_KONTAINER !== null) {
                    var B_KONTAINER = Ext.decode(val.B_KONTAINER, true);
                    var GRIDbcin_40_kontainer = popup.query("grid[pid=GRIDbcin_40_kontainer]")[0];
                    Ext.Array.each(B_KONTAINER, function (rec) {
                      GRIDbcin_40_kontainer.getStore().add(rec);
                    });
                  }
                  if (val.B_KEMASAN !== null) {
                    var B_KEMASAN = Ext.decode(val.B_KEMASAN, true);
                    var GRIDbcin_40_kemasan = popup.query("grid[pid=GRIDbcin_40_kemasan]")[0];
                    Ext.Array.each(B_KEMASAN, function (rec) {
                      GRIDbcin_40_kemasan.getStore().add(rec);
                    });
                  }

                  if (val.B_BARANG !== null) {
                    var B_BARANG = Ext.decode(val.B_BARANG, true);
                    var GRIDbcin_40_barang = popup.query("grid[pid=GRIDbcin_40_barang]")[0];
                    Ext.Array.each(B_BARANG, function (rec) {
                      GRIDbcin_40_barang.getStore().add(rec);
                    });
                  }
                } else {
  ...
  /*
  FRMbcin_40_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      var GRIDkontainer = cmp.query("form grid[pid=GRIDbcin_40_kontainer]")[0];
      var GRIDkemasan = cmp.query("form grid[pid=GRIDbcin_40_kemasan]")[0];
      var GRIDdokumen = cmp.query("form grid[pid=GRIDbcin_40_input_dokumen]")[0];
      var GRIDbarang = cmp.query("form grid[pid=GRIDbcin_40_input_item]")[0];
  ...
  */
  FRMbcin_40_btcancel_click: function () {
    try {
      var popup = Ext.ComponentQuery.query("FRMbcin_40")[0];
      var GRID = Ext.ComponentQuery.query("bcin_40 GRIDbcin_40 grid[pid=GRIDbcin_40]")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMbcin_40 form")[0];
  ...
  FRMbcin_40_btsendtoceisa_click: function () {
    try {
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcin_40_item", "NJC.EXIM.bcin_40.FRMbcin_40_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcin_40_item_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      ...
      var GRIDitem = Ext.ComponentQuery.query("FRMbcin_40 form grid[pid=GRIDbcin_40_input_item]")[0];
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_40.popup_btsearch", {
            ...
            listeners: {
              itemdblclick: function (grid, rec) {
                switch (btn.vdata.modulename) {
                  case "kantorasal":
                    var FRM = Ext.ComponentQuery.query("FRMbcin_40 form")[0];
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var griddokumen = Ext.ComponentQuery.query("FRMbcin_40 grid[pid=GRIDbcin_40_input_dokumen]")[0];

      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("NJC.EXIM.bcin_40.popup_btadddokumen", {
        extend: "Ext.window.Window",
   ...
   btupdate_click: function () {
    try {
      var me = this;
      var MAIN_FRM = Ext.ComponentQuery.query("FRMbcin_40")[0];
      var GRID = Ext.ComponentQuery.query("GRIDbcin_40 grid[pid=GRIDbcin_40]")[0];
  ...
  proses_update: function () {
    try {
      ...
      var GRIDbcin_40_pengangkut = MAIN_FRM.query("grid[pid=GRIDbcin_40_pengangkut]")[0];
      var STpengangkut = GRIDbcin_40_pengangkut.getStore().getData();
      var DATA_pengangkut = [];
      STpengangkut.each(function (rec) {
        DATA_pengangkut.push(rec.data);
      });

      var GRIDbcin_40_kontainer = MAIN_FRM.query("grid[pid=GRIDbcin_40_kontainer]")[0];
      var STkontainer = GRIDbcin_40_kontainer.getStore().getData();
      var DATA_kontainer = [];
      STkontainer.each(function (rec) {
        DATA_kontainer.push(rec.data);
      });

      var GRIDbcin_40_kemasan = MAIN_FRM.query("grid[pid=GRIDbcin_40_kemasan]")[0];
      var STkemasan = GRIDbcin_40_kemasan.getStore().getData();
      var DATA_kemasan = [];
      STkemasan.each(function (rec) {
        DATA_kemasan.push(rec.data);
      });

      var GRIDbcin_40_dokumen = MAIN_FRM.query("grid[pid=GRIDbcin_40_dokumen]")[0];
      var STdokumen = GRIDbcin_40_dokumen.getStore().getData();
      var DATA_dokumen = [];

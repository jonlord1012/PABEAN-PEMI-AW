Ext.define("TDK.EXIM.bcout_scrap.Cbcout_scrap", {
  ...
  GRIDbcout_scrap_itemdblclick: function (cmp, rec) {
    try {
      ...
      var vdt = rec.data;
      var load_edit = COMP.run.getmodulepopup("FRMbcout_scrap", "TDK.EXIM.bcout_scrap.FRMbcout_scrap", mainpanel);
      if (load_edit) {
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcout_scrap", "TDK.EXIM.bcout_scrap.FRMbcout_scrap", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },

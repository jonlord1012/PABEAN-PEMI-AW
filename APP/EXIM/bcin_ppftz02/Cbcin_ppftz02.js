Ext.define("NJC.EXIM.bcin_ppftz02.Cbcin_ppftz02", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbcin_ppftz02",
  init: function (view) {
    this.control({
      "bcin_ppftz02 button[pid=btrefresh]": { click: this.btrefresh_click },
      "bcin_ppftz02 combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      "FRMbcin_ppftz02 button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
      "FRMbcin_ppftz02 button[pid=btsearch]": { click: this.btsearch_click },
    });
    this.listen({
      store: {},
    });
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcin_kurir20", "NJC.EXIM.bcin_kurir20.FRMbcin_kurir20", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcin_kurir20_item", "NJC.EXIM.bcin_kurir20.FRMbcin_kurir20_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("NJC.EXIM.bcin_kurir20.popup_btsearch", {
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("NJC.EXIM.bcin_kurir20.popup_btadddokumen", {
        extend: "Ext.window.Window",

Ext.define("TDK.EXIM.bcin_ppb.Cbcin_ppb", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbcin_ppb",
  init: function (view) {
    this.control({
      "bcin_ppb button[pid=btrefresh]": { click: this.btrefresh_click },
      "bcin_ppb combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      "FRMbcin_ppb button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
      "FRMbcin_ppb button[pid=btsearch]": { click: this.btsearch_click },
    });
    this.listen({
      store: {},
    });
    this.var_global = {
      jwt: localStorage.getItem("ST_NJC_JWT_PLB_PLB"),
    };
    this.var_definition = {};
    this.renderpage();
  },
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cbcin_ppb");
    } catch (ex) {
  ...
  btrefresh_click: function () {
    try {
      var GRID = Ext.ComponentQuery.query("bcin_ppb GRIDbcin_ppb grid[pid=GRIDbcin_ppb]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcin_ppb", "TDK.EXIM.bcin_ppb.FRMbcin_ppb", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcin_ppb_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      var GRIDkontainer = cmp.query("form grid[pid=GRIDbcin_ppb_kontainer]")[0];
      var GRIDkemasan = cmp.query("form grid[pid=GRIDbcin_ppb_kemasan]")[0];
      var GRIDdokumen = cmp.query("form grid[pid=GRIDbcin_ppb_input_dokumen]")[0];
      var GRIDbarang = cmp.query("form grid[pid=GRIDbcin_ppb_input_item]")[0];
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "bcin_ppb/bcin_ppb", params, "POST", me.var_global.jwt);
  ...
  FRMbcin_ppb_btcancel_click: function () {
    try {
      var popup = Ext.ComponentQuery.query("FRMbcin_ppb")[0];
      var GRID = Ext.ComponentQuery.query("bcin_ppb GRIDbcin_ppb grid[pid=GRIDbcin_ppb]")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMbcin_ppb form")[0];
  ...
  FRMbcin_ppb_btsendtoceisa_click: function () {
    try {
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcin_ppb_item", "TDK.EXIM.bcin_ppb.FRMbcin_ppb_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcin_ppb_item_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      ...
      var GRIDitem = Ext.ComponentQuery.query("FRMbcin_ppb form grid[pid=GRIDbcin_ppb_input_item]")[0];
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_ppb.popup_btsearch", {
            ...
            listeners: {
              itemdblclick: function (grid, rec) {
                switch (btn.vdata.modulename) {
                  case "kantorpabean":
                    var FRM = Ext.ComponentQuery.query("FRMbcin_ppb form")[0];
    ...
    try {
      switch (modulename) {
        ...
        case "tujuanpemasukan":
          return [
            { xtype: "rownumberer", width: 40 },
            { header: "KODE TUJUAN PEMASUKAN", dataIndex: "KODE_TUJUAN_PEMASUKAN", sortable: true, width: 100, filter: { xtype: "textfield" } },
            { header: "NAMA TUJUAN PEMASUKAN", dataIndex: "URAIAN_TUJUAN_PEMASUKAN", sortable: true, width: 350, filter: { xtype: "textfield" } },
          ];
        case "valuta":
          return [
            { xtype: "rownumberer", width: 40 },
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var griddokumen = Ext.ComponentQuery.query("FRMbcin_ppb grid[pid=GRIDbcin_ppb_input_dokumen]")[0];

      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_ppb.popup_btadddokumen", {
        extend: "Ext.window.Window",

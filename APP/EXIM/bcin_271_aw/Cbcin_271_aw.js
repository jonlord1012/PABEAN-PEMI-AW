Ext.define("NJC.EXIM.bcin_271_aw.Cbcin_271_aw", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbcin_271_aw",
  init: function (view) {
    this.control({
      "bcin_271_aw button[pid=btrefresh]": { click: this.btrefresh_click },
      "bcin_271_aw combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      // "FRMbcin_271 button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
      // "FRMbcin_271 button[pid=btsearch]": { click: this.btsearch_click },
    });
    this.listen({
      store: {},
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cbcin_271_aw");
    } catch (ex) {
  ...
  btrefresh_click: function () {
    try {
      var GRID = Ext.ComponentQuery.query("bcin_271_aw GRIDbcin_271_aw grid[pid=GRIDbcin_271_aw]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcin_271", "NJC.EXIM.bcin_271.FRMbcin_271", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  */
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      ...
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.ComponentQuery.query("FRMbcin_271_aw")[0];
      if (!popup) {
        popup = Ext.create("NJC.EXIM.bcin_271_aw.FRMbcin_271_aw");
        popup.on(
          "show",
          function () {
            try {
              ...
              var hasil = COMP.run.getservice(vconfig.service_api + "bcin_271_aw/bcin_271_aw", params, "POST", me.var_global.jwt);
                ...
                if (val.success == "true") {
                  ...
                  if (val.B_HEADER !== null) {
                    ...
                    FRM.getForm().setValues({
                      ...
                      ASAL_DATA: B_HEADER.ASALDATA ? B_HEADER.ASALDATA : 'S',
                      SERI: B_HEADER.SERI ? B_HEADER.SERI : 0,
                      KODE_DOKUMEN_PABEAN: B_HEADER.KODEDOKUMEN,
                      MODE_SOURCE: B_HEADER.MODE_SOURCE ? B_HEADER.MODE_SOURCE : 'BICC',
                      ID_COMPANY: B_HEADER.ID_COMPANY ? B_HEADER.ID_COMPANY : 'AW',

                      POSTINGSTATUS: B_HEADER.POSTINGSTATUS ? B_HEADER.POSTINGSTATUS : "OPEN",
                      POSTINGUSER: B_HEADER.UPDATE_BY ? B_HEADER.UPDATE_BY : "",
                      POSTINGDATE: B_HEADER.UPDATE_DATE ? B_HEADER.UPDATE_DATE : "",
                      NOMOR_RESPON: B_HEADER.FLAG05 ? B_HEADER.FLAG05 : "--",
                      PORTAL_STATUS: B_HEADER.FLAG05 ? B_HEADER.FLAG05 : "---",
                      ...
                      KODE_KANTORNAME: B_HEADER.KODEKANTORNAME,
                      KODE_KANTOR_BONGKAR: B_HEADER.KODEKANTORBONGKAR,
                      KODE_KANTOR_BONGKARNAME: B_HEADER.KODEKANTORBONGKARNAME,
                      KODE_KANTOR_TUJUAN: B_HEADER.KODEKANTORTUJUAN,
                      KODE_KANTOR_TUJUANNAME: B_HEADER.KODEKANTORTUJUANNAME,
                      KODE_JENIS_TPB: B_HEADER.KODEJENISTPB,
                      KODE_JENIS_TPBNAME: B_HEADER.KODEJENISTPBNAME,
                      KODE_TUJUAN_TPB: B_HEADER.KODETUJUANTPB,
                      KODE_TUJUAN_TPBNAME: B_HEADER.KODETUJUANTPBNAME,
                      KODE_TUJUAN_PEMASUKAN: B_HEADER.KODETUJUANPEMASUKAN,
                      KODE_TUJUAN_PEMASUKANNAME: B_HEADER.KODETUJUANPEMASUKANNAME,
                      KODE_TUJUAN_PENGIRIMAN: B_HEADER.KODETUJUANPENGIRIMAN,
                      KODE_TUJUAN_PENGIRIMANNAME: B_HEADER.KODETUJUANPENGIRIMANNAME,

                      KODE_TPS: B_HEADER.KODETPS,
                      KODE_TPS_NAME: B_HEADER.KODETPSNAME,

                      KODE_VALUTA: B_HEADER.KODEVALUTA ? B_HEADER.KODEVALUTA : "IDR",
                      KODE_VALUTA_NAME: B_HEADER.KODEVALUTANAME ? B_HEADER.KODEVALUTANAME : "RUPIAH",
                      FOB: B_HEADER.FOB ? B_HEADER.FOB : 0,
                      NDPBM: B_HEADER.NDPBM ? B_HEADER.NDPBM : 0,
                      CIF: B_HEADER.CIF ? B_HEADER.CIF : 0,
                      CIF_RUPIAH: B_HEADER.CIFRUPIAH ? B_HEADER.CIFRUPIAH : B_HEADER.HARGAPENYERAHAN,
                      ...
                      KODE_PEL_MUATNAME: B_HEADER.KODEPELABUHANMUATNAME,
                      KODE_PEL_TRANSIT: B_HEADER.KODEPELABUHANTRANSIT,
                      KODE_PEL_TRANSITNAME: B_HEADER.KODEPELABUHANTRANSITNAME,
                      KODE_PEL_BONGKAR: B_HEADER.KODEPELABUHANBONGKAR,
                      KODE_PEL_BONGKARNAME: B_HEADER.KODEPELABUHANBONGKARNAME,
                    });
                  }

                  if (val.B_DOKUMEN !== null) {
                    var B_DOKUMEN = Ext.decode(val.B_DOKUMEN, true);
                    var GRIDbcin_271_aw_dokumen = popup.query("grid[pid=GRIDbcin_271_aw_dokumen]")[0];
                    Ext.Array.each(B_DOKUMEN, function (rec) {
                      GRIDbcin_271_aw_dokumen.getStore().add(rec);
                    });
                  }
                  ...
                  if (val.B_PUNGUTAN !== null) {
                    var B_PUNGUTAN = Ext.decode(val.B_PUNGUTAN, true);
                    var GRIDbcin_271_aw_pungutan = popup.query("grid[pid=GRIDbcin_271_aw_pungutan]")[0];
                    Ext.Array.each(B_PUNGUTAN, function (rec) {
                      GRIDbcin_271_aw_pungutan.getStore().add(rec);
                    });
                  }
                  if (val.B_PENGANGKUT !== null) {
                    var B_PENGANGKUT = Ext.decode(val.B_PENGANGKUT, true);
                    var GRIDbcin_271_aw_pengangkut = popup.query("grid[pid=GRIDbcin_271_aw_pengangkut]")[0];
                    Ext.Array.each(B_PENGANGKUT, function (rec) {
                      GRIDbcin_271_aw_pengangkut.getStore().add(rec);
                    });
                  }
                  if (val.B_KONTAINER !== null) {
                    var B_KONTAINER = Ext.decode(val.B_KONTAINER, true);
                    var GRIDbcin_271_aw_kontainer = popup.query("grid[pid=GRIDbcin_271_aw_kontainer]")[0];
                    Ext.Array.each(B_KONTAINER, function (rec) {
                      GRIDbcin_271_aw_kontainer.getStore().add(rec);
                    });
                  }
                  if (val.B_KEMASAN !== null) {
                    var B_KEMASAN = Ext.decode(val.B_KEMASAN, true);
                    var GRIDbcin_271_aw_kemasan = popup.query("grid[pid=GRIDbcin_271_aw_kemasan]")[0];
                    Ext.Array.each(B_KEMASAN, function (rec) {
                      GRIDbcin_271_aw_kemasan.getStore().add(rec);
                    });
                  }

                  if (val.B_BARANG !== null) {
                    var B_BARANG = Ext.decode(val.B_BARANG, true);
                    var GRIDbcin_271_aw_barang = popup.query("grid[pid=GRIDbcin_271_aw_barang]")[0];
                    Ext.Array.each(B_BARANG, function (rec) {
                      GRIDbcin_271_aw_barang.getStore().add(rec);
                    });
                  }
                } else {
  ...
  /*
  FRMbcin_271_aw_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      var GRIDkontainer = cmp.query("form grid[pid=GRIDbcin_271_aw_kontainer]")[0];
      var GRIDkemasan = cmp.query("form grid[pid=GRIDbcin_271_aw_kemasan]")[0];
      var GRIDdokumen = cmp.query("form grid[pid=GRIDbcin_271_aw_input_dokumen]")[0];
      var GRIDbarang = cmp.query("form grid[pid=GRIDbcin_271_aw_input_item]")[0];
  ...
  */
  FRMbcin_271_aw_btcancel_click: function () {
    try {
      var popup = Ext.ComponentQuery.query("FRMbcin_271_aw")[0];
      var GRID = Ext.ComponentQuery.query("bcin_271_aw GRIDbcin_271_aw grid[pid=GRIDbcin_271_aw]")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMbcin_271_aw form")[0];
  ...
  FRMbcin_271_aw_btsendtoceisa_click: function () {
    try {
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcin_271_aw_item", "NJC.EXIM.bcin_271_aw.FRMbcin_271_aw_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcin_271_aw_item_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      ...
      var GRIDitem = Ext.ComponentQuery.query("FRMbcin_271_aw form grid[pid=GRIDbcin_271_aw_input_item]")[0];
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_271_aw.popup_btsearch", {
        ...
        case "tujuanpengiriman27":
          return {
            ...
            proxy: {
              ...
              headers: { Authorization: "Bearer " + localStorage.getItem("ST_NJC_JWT_PLB") },
              actionMethods: { read: "POST" },
              url: vconfig.service_api + "referensi_tujuan_pengiriman27/referensi_tujuan_pengiriman27s",
              extraParams: {
                method: "read",
                module: "bcin271",
              },
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var griddokumen = Ext.ComponentQuery.query("FRMbcin_271_aw grid[pid=GRIDbcin_271_aw_input_dokumen]")[0];

      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("NJC.EXIM.bcin_271_aw.popup_btadddokumen", {
        extend: "Ext.window.Window",
  ...
  btupdate_click: function () {
    try {
      var me = this;
      var MAIN_FRM = Ext.ComponentQuery.query("FRMbcin_271_aw")[0];
      var GRID = Ext.ComponentQuery.query("GRIDbcin_271_aw grid[pid=GRIDbcin_271_aw]")[0];
  ...
  proses_update: function () {
    try {
      ...
      var GRIDbcin_271_aw_pengangkut = MAIN_FRM.query("grid[pid=GRIDbcin_271_aw_pengangkut]")[0];
      var STpengangkut = GRIDbcin_271_aw_pengangkut.getStore().getData();
      var DATA_pengangkut = [];
      STpengangkut.each(function (rec) {
        DATA_pengangkut.push(rec.data);
      });

      var GRIDbcin_271_aw_kontainer = MAIN_FRM.query("grid[pid=GRIDbcin_271_aw_kontainer]")[0];
      var STkontainer = GRIDbcin_271_aw_kontainer.getStore().getData();
      var DATA_kontainer = [];
      STkontainer.each(function (rec) {
        DATA_kontainer.push(rec.data);
      });

      var GRIDbcin_271_aw_kemasan = MAIN_FRM.query("grid[pid=GRIDbcin_271_aw_kemasan]")[0];
      var STkemasan = GRIDbcin_271_aw_kemasan.getStore().getData();
      var DATA_kemasan = [];
      STkemasan.each(function (rec) {
        DATA_kemasan.push(rec.data);
      });

      var GRIDbcin_271_aw_dokumen = MAIN_FRM.query("grid[pid=GRIDbcin_271_aw_dokumen]")[0];
      var STdokumen = GRIDbcin_271_aw_dokumen.getStore().getData();
      var DATA_dokumen = [];

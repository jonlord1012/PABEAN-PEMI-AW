Ext.define("TDK.EXIM.bcin_kurir23.Cbcin_kurir23", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbcin_kurir23",
  init: function (view) {
    this.control({
      "bcin_kurir23 button[pid=btrefresh]": { click: this.btrefresh_click },
      "bcin_kurir23 combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      "FRMbcin_kurir23 button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
      "FRMbcin_kurir23 button[pid=btsearch]": { click: this.btsearch_click },
    });
    this.listen({
      store: {},
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cbcin_kurir23");
    } catch (ex) {
  ...
  btrefresh_click: function () {
    try {
      var GRID = Ext.ComponentQuery.query("bcin_kurir23 GRIDbcin_kurir23 grid[pid=GRIDbcin_kurir23]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcin_kurir23", "TDK.EXIM.bcin_kurir23.FRMbcin_kurir23", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcin_kurir23_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      var GRIDkontainer = cmp.query("form grid[pid=GRIDbcin_kurir23_kontainer]")[0];
      var GRIDkemasan = cmp.query("form grid[pid=GRIDbcin_kurir23_kemasan]")[0];
      var GRIDdokumen = cmp.query("form grid[pid=GRIDbcin_kurir23_input_dokumen]")[0];
      var GRIDbarang = cmp.query("form grid[pid=GRIDbcin_kurir23_input_item]")[0];
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "bcin_kurir23/bcin_kurir23", params, "POST", me.var_global.jwt);
  ...
  FRMbcin_kurir23_btcancel_click: function () {
    try {
      var popup = Ext.ComponentQuery.query("FRMbcin_kurir23")[0];
      var GRID = Ext.ComponentQuery.query("bcin_kurir23 GRIDbcin_kurir23 grid[pid=GRIDbcin_kurir23]")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMbcin_kurir23 form")[0];
  ...
  FRMbcin_kurir23_btsendtoceisa_click: function () {
    try {
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcin_kurir23_item", "TDK.EXIM.bcin_kurir23.FRMbcin_kurir23_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcin_kurir23_item_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      ...
      var GRIDitem = Ext.ComponentQuery.query("FRMbcin_kurir23 form grid[pid=GRIDbcin_kurir23_input_item]")[0];
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_kurir23.popup_btsearch", {
            ...
            listeners: {
              itemdblclick: function (grid, rec) {
                switch (btn.vdata.modulename) {
                  case "negara":
                    var FRM = Ext.ComponentQuery.query("FRMbcin_kurir23 form")[0];
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var griddokumen = Ext.ComponentQuery.query("FRMbcin_kurir23 grid[pid=GRIDbcin_kurir23_input_dokumen]")[0];

      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_kurir23.popup_btadddokumen", {
        extend: "Ext.window.Window",

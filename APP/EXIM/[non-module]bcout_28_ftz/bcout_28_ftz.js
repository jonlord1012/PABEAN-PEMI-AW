Ext.define("TDK.EXIM.bcout_28_ftz.bcout_28_ftz", {
  extend: "Ext.form.Panel",
  alias: "widget.bcout_28_ftz",
  reference: "bcout_28_ftz",
  config: {},
  requires: ["TDK.EXIM.bcout_28_ftz.GRIDbcout_28_ftz", "TDK.EXIM.bcout_28_ftz.Cbcout_28_ftz"],
  constructor: function (config) {
    return this.callParent(arguments);
  },
  //untuk include controller
  controller: "Cbcout_28_ftz",
  initComponent: function () {
    Ext.apply(this, {
      xtype: "panel",
      pid: "panelbcout_28_ftz",
      layout: "card",
      frame: false,
      border: false,
      items: [{ xtype: "GRIDbcout_28_ftz" }],
      dockedItems: [
        {
          xtype: "toolbar",
          height: 30,
          dock: "top",
          items: [
            "-",
            {
              xtype: "combobox",
              name: "CBO_FILTERKEY",
              fieldLabel: "Filter Data Key",
              labelWidth: 80,
              width: 350,
              displayField: "DEFNAME",
              valueField: "DEFCODE",
              fieldCls: "fieldinput",
              allowBlank: false,
              queryMode: "local",
              forceSelection: true,
              typeAhead: true,
              minChars: 0,
              anyMatch: true,
              value: "0",
              store: new Ext.data.Store({
                data: [
                  { DEFCODE: "0", DEFNAME: "All Data" },
                  { DEFCODE: "1", DEFNAME: "Dokumen Masih Proses Draft" },
                  { DEFCODE: "2", DEFNAME: "Dokumen Belum SPPD" },
                ],
                fields: ["DEFCODE", "DEFNAME"],
              }),
            },
            "-",
            { xtype: "button", pid: "btrefresh", icon: vconfig.getstyle + "icon/update.ico", tooltip: "Refresh Data" },
            "-",
          ],
          // other options....
        },
      ],
    });

    this.callParent(arguments);
  },
});

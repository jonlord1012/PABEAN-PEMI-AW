Ext.define("NJC.EXIM.bcout_27.Cbcout_27", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbcout_27",
  init: function (view) {
    this.control({
      "bcout_27 button[pid=btrefresh]": { click: this.btrefresh_click },
      "bcout_27 combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      // "FRMbcout_27 button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
      // "FRMbcout_27 button[pid=btsearch]": { click: this.btsearch_click },
    });
    this.listen({
      store: {},
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cbcout_27");
    } catch (ex) {
  ...
  btrefresh_click: function () {
    try {
      var GRID = Ext.ComponentQuery.query("bcout_27 GRIDbcout_27 grid[pid=GRIDbcout_27]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcout_27", "NJC.EXIM.bcout_27.FRMbcout_27", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  */
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      ...
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.ComponentQuery.query("FRMbcout_27")[0];
      if (!popup) {
        popup = Ext.create("NJC.EXIM.bcout_27.FRMbcout_27");
        popup.on(
          "show",
          function () {
            try {
                ...
                if (val.success == "true") {
                  ...
                  if (val.B_HEADER !== null) {
                    ...
                    FRM.getForm().setValues({
                      ...
                      MODE_SOURCE: B_HEADER.MODE_SOURCE ? B_HEADER.MODE_SOURCE : 'BINLOC',
                      ID_COMPANY: B_HEADER.ID_COMPANY ? B_HEADER.ID_COMPANY : 'PLB',

                      POSTINGSTATUS: B_HEADER.POSTINGSTATUS ? B_HEADER.POSTINGSTATUS : "OPEN",
                      POSTINGUSER: B_HEADER.UPDATE_BY ? B_HEADER.UPDATE_BY : "",
                      POSTINGDATE: B_HEADER.UPDATE_DATE ? moment(B_HEADER.UPDATE_DATE).format("YYYY-MM-DD") : "",
                      ...
                      KODE_CARA_BAYAR: B_HEADER.KODE_CARA_BAYAR,
                      KODE_CARA_BAYARNAME: B_HEADER.KODE_CARA_BAYARNAME,
                      KODE_TUTUP_PU: B_HEADER.KODETUTUPPU,
                      NOMOR_BC11: B_HEADER.NOMORBC11,
                    });
                  }

                  if (val.B_DOKUMEN !== null) {
                    var B_DOKUMEN = Ext.decode(val.B_DOKUMEN, true);
                    var GRIDbcout_27_dokumen = popup.query("grid[pid=GRIDbcout_27_dokumen]")[0];
                    Ext.Array.each(B_DOKUMEN, function (rec) {
                      GRIDbcout_27_dokumen.getStore().add(rec);
                    });
                  }
                  ...
                  if (val.B_PUNGUTAN !== null) {
                    ...
                    var GRIDbcout_27_pungutan = popup.query("grid[pid=GRIDbcout_27_pungutan]")[0];
                    Ext.Array.each(B_PUNGUTAN, function (rec) {
                      GRIDbcout_27_pungutan.getStore().add(rec);
                    });
                  }
                  if (val.B_PENGANGKUT !== null) {
                    var B_PENGANGKUT = Ext.decode(val.B_PENGANGKUT, true);
                    var GRIDbcout_27_pengangkut = popup.query("grid[pid=GRIDbcout_27_pengangkut]")[0];
                    Ext.Array.each(B_PENGANGKUT, function (rec) {
                      GRIDbcout_27_pengangkut.getStore().add(rec);
                    });
                  }
                  if (val.B_KONTAINER !== null) {
                    var B_KONTAINER = Ext.decode(val.B_KONTAINER, true);
                    var GRIDbcout_27_kontainer = popup.query("grid[pid=GRIDbcout_27_kontainer]")[0];
                    Ext.Array.each(B_KONTAINER, function (rec) {
                      GRIDbcout_27_kontainer.getStore().add(rec);
                    });
                  }
                  if (val.B_KEMASAN !== null) {
                    var B_KEMASAN = Ext.decode(val.B_KEMASAN, true);
                    var GRIDbcout_27_kemasan = popup.query("grid[pid=GRIDbcout_27_kemasan]")[0];
                    Ext.Array.each(B_KEMASAN, function (rec) {
                      GRIDbcout_27_kemasan.getStore().add(rec);
                    });
                  }

                  if (val.B_BARANG !== null) {
                    var B_BARANG = Ext.decode(val.B_BARANG, true);
                    var GRIDbcout_27_barang = popup.query("grid[pid=GRIDbcout_27_barang]")[0];
                    Ext.Array.each(B_BARANG, function (rec) {
                      GRIDbcout_27_barang.getStore().add(rec);
                    });
                  }
                } else {
  ...
  /*
  FRMbcout_27_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      var GRIDkontainer = cmp.query("form grid[pid=GRIDbcout_27_kontainer]")[0];
      var GRIDkemasan = cmp.query("form grid[pid=GRIDbcout_27_kemasan]")[0];
      var GRIDdokumen = cmp.query("form grid[pid=GRIDbcout_27_input_dokumen]")[0];
      var GRIDbarang = cmp.query("form grid[pid=GRIDbcout_27_input_item]")[0];
  ...
  */
  FRMbcout_27_btcancel_click: function () {
    try {
      var popup = Ext.ComponentQuery.query("FRMbcout_27")[0];
      var GRID = Ext.ComponentQuery.query("bcout_27 GRIDbcout_27 grid[pid=GRIDbcout_27]")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMbcout_27 form")[0];
  ...
  FRMbcout_27_btsendtoceisa_click: function () {
    try {
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcout_27_item", "NJC.EXIM.bcout_27.FRMbcout_27_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  FRMbcout_27_item_load: function (cmp) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = cmp.query("form")[0];
      ...
      var GRIDitem = Ext.ComponentQuery.query("FRMbcout_27 form grid[pid=GRIDbcout_27_input_item]")[0];
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("NJC.EXIM.bcout_27.popup_btsearch", {
            ...
            listeners: {
              itemdblclick: function (grid, rec) {
                switch (btn.vdata.modulename) {
                  case "kantorasal":
                    var FRM = Ext.ComponentQuery.query("FRMbcout_27 form")[0];
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var griddokumen = Ext.ComponentQuery.query("FRMbcout_27 grid[pid=GRIDbcout_27_input_dokumen]")[0];

      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("NJC.EXIM.bcout_27.popup_btadddokumen", {
        extend: "Ext.window.Window",
  ...
  btupdate_click: function () {
    try {
      var me = this;
      var MAIN_FRM = Ext.ComponentQuery.query("FRMbcout_27")[0];
      var GRID = Ext.ComponentQuery.query("GRIDbcout_27 grid[pid=GRIDbcout_27]")[0];
  ...
  proses_update: function () {
    try {
      ...
      var GRIDbcout_27_pengangkut = MAIN_FRM.query("grid[pid=GRIDbcout_27_pengangkut]")[0];
      var STpengangkut = GRIDbcout_27_pengangkut.getStore().getData();
      var DATA_pengangkut = [];
      STpengangkut.each(function (rec) {
        DATA_pengangkut.push(rec.data);
      });

      var GRIDbcout_27_kontainer = MAIN_FRM.query("grid[pid=GRIDbcout_27_kontainer]")[0];
      var STkontainer = GRIDbcout_27_kontainer.getStore().getData();
      var DATA_kontainer = [];
      STkontainer.each(function (rec) {
        DATA_kontainer.push(rec.data);
      });

      var GRIDbcout_27_kemasan = MAIN_FRM.query("grid[pid=GRIDbcout_27_kemasan]")[0];
      var STkemasan = GRIDbcout_27_kemasan.getStore().getData();
      var DATA_kemasan = [];
      STkemasan.each(function (rec) {
        DATA_kemasan.push(rec.data);
      });

      var GRIDbcout_27_dokumen = MAIN_FRM.query("grid[pid=GRIDbcout_27_dokumen]")[0];
      var STdokumen = GRIDbcout_27_dokumen.getStore().getData();
      var DATA_dokumen = [];

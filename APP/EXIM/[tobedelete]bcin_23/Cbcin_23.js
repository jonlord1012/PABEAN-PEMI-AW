Ext.define("TDK.EXIM.bcin_23.Cbcin_23", {
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      COMP.run.getmodulepopup("FRMbcin_23", "TDK.EXIM.bcin_23.FRMbcin_23", mainpanel);
      ...
      hasil.then(function (content) {
        ...
        if (val.success === "true") {
          if (val.vheader !== null) {
            ...
            if(dtheader.KODE_STATUS!=='80'){
              Ext.getCmp("btcancel_dokumen").setDisabled(false);
              Ext.getCmp("btsimpan_draft").setDisabled(false);
              Ext.getCmp("btlock_draft").setDisabled(false);
              Ext.getCmp("btsend_to_ceisa").setDisabled(false);
              Ext.getCmp("bt_get_from_ceisa").setDisabled(false);
              console.log("unlockme please");
            }
            else{
              Ext.getCmp("btcancel_dokumen").setDisabled(true);
              Ext.getCmp("btsimpan_draft").setDisabled(true);
              Ext.getCmp("btlock_draft").setDisabled(true);
              Ext.getCmp("btsend_to_ceisa").setDisabled(true);
              Ext.getCmp("bt_get_from_ceisa").setDisabled(true);
              console.log("lockme please");
            }
            FRM.getForm().setValues(dtheader);
            FRM.getForm().setValues({
              KODE_CARA_ANGKUT: dtheader.KODE_CARA_ANGKUT,
            });
          }
  ...
  getButtonStatus: function (cmp) {
    try {
      var FRM_MAIN = cmp.query("form")[0];
      var dtval = FRM_MAIN.getValues(false, false, false, true);
      
      console.log(dtval.ID_HEADER_ORI + 'adklafa');
      
      if(dtval.KODE_STATUS==='80'){
        var btnCancelDokumen = Ext.ComponentQuery.query("bcin_23 FRMbcin_23 button[pid=btcancel_dokumen]")[0];
        var btnSimpanDraft = Ext.ComponentQuery.query("bcin_23 FRMbcin_23 button[pid=btsimpan_draft]")[0];
        var btnLockDraft = Ext.ComponentQuery.query("bcin_23 FRMbcin_23 button[pid=btlock_draft]")[0];
        var btnSendCeisa = Ext.ComponentQuery.query("bcin_23 FRMbcin_23 button[pid=btsend_to_ceisa]")[0];
        var btnGetCeisa = Ext.ComponentQuery.query("bcin_23 FRMbcin_23 button[pid=bt_get_from_ceisa]")[0];
        Ext.getCmp(btnCancelDokumen).setDisabled(true);
        Ext.getCmp(btnSimpanDraft).setDisabled(true);
        Ext.getCmp(btnLockDraft).setDisabled(true);
        Ext.getCmp(btnSendCeisa).setDisabled(true);
        Ext.getCmp(btnGetCeisa).setDisabled(true);
        /*
        btnCancelDokumen.setDisabled(true);
        btnSimpanDraft.setDisabled(true);
        btnLockDraft.setDisabled(true);
        btnSendCeisa.setDisabled(true);
        btnGetCeisa.setDisabled(true);
        */
      }
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
  ...
  FRMbcin_23_btgetfromceisa_click: function() {
    try {
      var popup = Ext.ComponentQuery.query("FRMbcin_23")[0];
      var GRID = Ext.ComponentQuery.query("bcin_23 GRIDbcin_23 grid[pid=GRIDbcin_23]")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMbcin_23 form")[0];
      ...
      Ext.MessageBox.confirm(
        ...
        //
        "<b>Syarat untuk Proses KIRIM Dokumen BC: </b>" +
          //
          "<ol>" +
          "<li>Dokumen belum terkirim ke Aplikasi CEISA</li>" +
          "<li>Dokumen belum proses Receiving (penerimaan Item/Part)</li>" +
        ...
        function (button) {
          if (button === "yes") {
            var params = Ext.encode({
              method: "proses_get_dokumen",
              MODULE: MAIN_dtval.MODE_SOURCE,
              INVOICE_NO: MAIN_dtval.INVOICE_NOMOR_DOKUMEN
            });
            var hasil = COMP.run.getservice(vconfig.service_api + "bcin_23/bcin_23", params, "POST", me.var_global.jwt);
            hasil.then(function (content) {
              var val = Ext.decode(content, true);
  ...
  edit_detailbarang_click: function (xgrid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMbcin_23_item", "TDK.EXIM.bcin_23.FRMbcin_23_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_23.popup_btsearch", {
  ...
  bttambah_dokumen_click: function () {
    try {
      ...
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.EXIM.bcin_23.popup_btadddokumen", {
        extend: "Ext.window.Window",
  ...
  FRMbcin_23_btupdate_click : function () {
    try {	  
        var me = this;
        ...
        var formValues = {
        MODE_SOURCE 						      : dtval.MODE_SOURCE 				      		/* sumber dokumen */
        , ID_HEADER_ORI 				      : dtval.ID_HEADER_ORI		    		  		/* dokumen id */
        , KODE_KANTOR_BONGKAR  				: dtval.KODE_KANTOR_BONGKAR						/* KANTOR BONGKAR  -- KODE_KANTOR_BONGKAR*/
        , KODE_KANTOR_PENGAWAS		    : dtval.KODE_KANTOR_PENGAWAS				  /* KODE_KANTOR_PENGAWAS */
        , KODE_TUJUAN_TPB       			: dtval.KODE_TUJUAN_TPB	        			/* KODE_TUJUAN_TPB */
        , KODE_CARA_ANGKUT						: dtval.KODE_CARA_ANGKUT				      /* 10 */
        , NAMA_PENGANGKUT				      : dtval.NAMA_PENGANGKUT  					    /* 11a */
        , NOMOR_VOY_FLIGHT					  : dtval.NOMOR_VOY_FLIGHT   	  				/* 11b */
        , KODE_BENDERA					      : dtval.KODE_BENDERA						      /* 11c  */
        , KODE_PEL_MUAT				        : dtval.KODE_PEL_MUAT					        /* 12 */
        , KODE_PEL_TRANSIT            : dtval.KODE_PEL_TRANSIT	            /* 13 */
        , KODE_PEL_BONGKAR					  : dtval.KODE_PEL_BONGKAR						  /* 14 */ 
        , KODE_ASURANSI			          : dtval.KODE_ASURANSI				          /* 22c */
        , NDPBM				                : dtval.NDPBM					                /* 23 */
        , FREIGHT                     : dtval.FREIGHT	                      /* 25 */
        , ASURANSI							      : dtval.ASURANSI							        /* 26 */
        , BRUTO							          : dtval.BRUTO													/* 30 */
        , NETTO							          : dtval.NETTO			                    /* 31 */
        , KODE_TPS										: dtval.KODE_TPS                      /* 21 */
        , JENIS_DOKUMEN_PABEAN			  : "23"										            /* EXTRA PARAMS */ 
        /* THESE PART ARE FOR DOKUMEN */
        , KODE_JENIS_DOKUMEN_LC		  	: '465'
        , NOMOR_DOKUMEN_LC	          : dtval.LC_NOMOR_DOKUMEN  
        , TANGGAL_DOKUMEN_LC          : dtval.LC_TANGGAL_DOKUMEN
        , URAIAN_DOKUMEN_LC		        : 'L/C'
        , KODE_JENIS_DOKUMEN_IMPORT		: dtval.IMPOR_KODE_DOKUMEN
        , NOMOR_DOKUMEN_IMPORT        : dtval.IMPOR_NOMOR_DOKUMEN
        , TANGGAL_DOKUMEN_IMPORT      : dtval.IMPOR_TANGGAL_DOKUMEN
        , URAIAN_DOKUMEN_IMPORT		    : dtval.IMPOR_URAIAN_DOKUMEN


      }; 
      
        var GRIDbcin_23_kemasan = Ext.ComponentQuery.query("FRMbcin_23 grid[pid=GRIDbcin_23_kemasan]")[0];
        var vkemasan = [];
        GRIDbcin_23_kemasan.getStore()
          .getDataSource()
          .each(function (record, index, count) {
            vkemasan.push({
                ID_KEMASAN			: record.data.ID_KEMASAN
              ...
              , ID_HEADER			: dtval.ID_HEADER_ORI
            });
          });

          var GRIDbcin_23_kontainer = Ext.ComponentQuery.query("FRMbcin_23 grid[pid=GRIDbcin_23_kontainer]")[0];
          var vkontainer = [];
          GRIDbcin_23_kontainer.getStore()
            .getDataSource()
            .each(function (record, index, count) {
              vkontainer.push({
                  ID	          	: record.data.ID
        ...
        var params = Ext.encode({
          method: "proses_update_bcin_23",
          header: Ext.encode(formValues),
          vkemasan: Ext.encode(vkemasan),
          vid_header_ori:  dtval.ID_HEADER_ORI, 
          vjenis_dokumen:'23',
          vkontainer: Ext.encode(vkontainer)
        });

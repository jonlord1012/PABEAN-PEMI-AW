Ext.define("NJC.EXIM.bcout_41_aw.Cbcout_41_aw", {
    extend: "Ext.app.ViewController",
    alias: "controller.Cbcout_41_aw",
    init: function (view) {
      this.control({
        "bcout_41_aw button[pid=btrefresh]": { click: this.btrefresh_click },
        "bcout_41_aw combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
        // "FRMbcout_41_aw button[pid=bttambah_dokumen]": { click: this.bttambah_dokumen_click },
        // "FRMbcout_41_aw button[pid=btsearch]": { click: this.btsearch_click },
      });
      this.listen({
        store: {},
      });
    ...
    renderpage: function () {
      try {
        console.log("load Controller Cbcout_41_aw");
      } catch (ex) {
    ...
    btrefresh_click: function () {
      try {
        var GRID = Ext.ComponentQuery.query("bcout_41_aw GRIDbcout_41_aw grid[pid=GRIDbcout_41_aw]")[0];
        GRID.getStore().load();
      } catch (ex) {
        COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
      }
    ...
    btdetail_rows_click: function (xgrid, rowIndex) {
      try {
        ...
        COMP.run.getmodulepopup("FRMbcout_41_aw", "NJC.EXIM.bcout_41_aw.FRMbcout_41_aw", mainpanel);
      } catch (ex) {
        COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
      }
    },
    */
    btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
      try {
        ...
        var vdt = rec.data;
        var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
        var popup = Ext.ComponentQuery.query("FRMbcout_41_aw")[0];
        if (!popup) {
          popup = Ext.create("NJC.EXIM.bcout_41_aw.FRMbcout_41_aw");
          popup.on(
            "show",
            function () {
              try {
                ...
                var hasil = COMP.run.getservice(vconfig.service_api + "bcout_41_aw/bcout_41_aw", params, "POST", me.var_global.jwt);
                hasil.then(function (content) {
                  ...
                  if (val.success == "true") {
                    ...
                    if (val.B_DOKUMEN !== null) {
                      ...
                      var GRIDbcout_41_aw_dokumen = popup.query("grid[pid=GRIDbcout_41_aw_dokumen]")[0];
                      Ext.Array.each(B_DOKUMEN, function (rec) {
                        GRIDbcout_41_aw_dokumen.getStore().add(rec);
                      });
                    }
                    ...
                    if (val.B_PENGANGKUT !== null) {
                      ...
                      var GRIDbcout_41_aw_pengangkut = popup.query("grid[pid=GRIDbcout_41_aw_pengangkut]")[0];
                      Ext.Array.each(B_PENGANGKUT, function (rec) {
                        GRIDbcout_41_aw_pengangkut.getStore().add(rec);
                      });
                    }
                    if (val.B_KONTAINER !== null) {
                      var B_KONTAINER = Ext.decode(val.B_KONTAINER, true);
                      var GRIDbcout_41_aw_kontainer = popup.query("grid[pid=GRIDbcout_41_aw_kontainer]")[0];
                      Ext.Array.each(B_KONTAINER, function (rec) {
                        GRIDbcout_41_aw_kontainer.getStore().add(rec);
                      });
                    }
                    if (val.B_KEMASAN !== null) {
                      var B_KEMASAN = Ext.decode(val.B_KEMASAN, true);
                      var GRIDbcout_41_aw_kemasan = popup.query("grid[pid=GRIDbcout_41_aw_kemasan]")[0];
                      Ext.Array.each(B_KEMASAN, function (rec) {
                        GRIDbcout_41_aw_kemasan.getStore().add(rec);
                      });
                    }
  
                    if (val.B_BARANG !== null) {
                      var B_BARANG = Ext.decode(val.B_BARANG, true);
                      var GRIDbcout_41_aw_barang = popup.query("grid[pid=GRIDbcout_41_aw_barang]")[0];
                      Ext.Array.each(B_BARANG, function (rec) {
                        GRIDbcout_41_aw_barang.getStore().add(rec);
                      });
                    }
                  } else {
    ...
    /*
    FRMbcout_41_aw_load: function (cmp) {
      try {
        var me = this;
        var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
        var FRM = cmp.query("form")[0];
        var GRIDkontainer = cmp.query("form grid[pid=GRIDbcout_41_aw_kontainer]")[0];
        var GRIDkemasan = cmp.query("form grid[pid=GRIDbcout_41_aw_kemasan]")[0];
        var GRIDdokumen = cmp.query("form grid[pid=GRIDbcout_41_aw_input_dokumen]")[0];
        var GRIDbarang = cmp.query("form grid[pid=GRIDbcout_41_aw_input_item]")[0];
    ...
    */
    FRMbcout_41_aw_btcancel_click: function () {
      try {
        var popup = Ext.ComponentQuery.query("FRMbcout_41_aw")[0];
        var GRID = Ext.ComponentQuery.query("bcout_41_aw GRIDbcout_41_aw grid[pid=GRIDbcout_41_aw]")[0];
        var FRM_MAIN = Ext.ComponentQuery.query("FRMbcout_41_aw form")[0];
    ...
    FRMbcout_41_aw_btsendtoceisa_click: function () {
      try {
    ...
    edit_detailbarang_click: function (xgrid, rowIndex) {
      try {
        ...
        var me = this;
        var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
        COMP.run.getmodulepopup("FRMbcout_41_aw_item", "NJC.EXIM.bcout_41_aw.FRMbcout_41_aw_item", mainpanel);
      } catch (ex) {
        COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
      }
    },
    FRMbcout_41_aw_item_load: function (cmp) {
      try {
        var me = this;
        var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
        var FRM = cmp.query("form")[0];
        ...
        var GRIDitem = Ext.ComponentQuery.query("FRMbcout_41_aw form grid[pid=GRIDbcout_41_aw_input_item]")[0];
    ...
    btsearch_click: function (btn) {
      try {
        var me = this;
        var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
        var popup = Ext.define("TDK.EXIM.bcout_41_aw.popup_btsearch", {
    ...
    bttambah_dokumen_click: function () {
      try {
        ...
        var griddokumen = Ext.ComponentQuery.query("FRMbcout_41_aw grid[pid=GRIDbcout_41_aw_input_dokumen]")[0];
  
        var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
        var popup = Ext.define("NJC.EXIM.bcout_41_aw.popup_btadddokumen", {
          extend: "Ext.window.Window",
     ...
     btupdate_click: function () {
      try {
        var me = this;
        var MAIN_FRM = Ext.ComponentQuery.query("FRMbcout_41_aw")[0];
        var GRID = Ext.ComponentQuery.query("GRIDbcout_41_aw grid[pid=GRIDbcout_41_aw]")[0];
    ...
    proses_update: function () {
      try {
        ...
        var GRIDbcout_41_aw_pengangkut = MAIN_FRM.query("grid[pid=GRIDbcout_41_aw_pengangkut]")[0];
        var STpengangkut = GRIDbcout_41_aw_pengangkut.getStore().getData();
        var DATA_pengangkut = [];
        STpengangkut.each(function (rec) {
          DATA_pengangkut.push(rec.data);
        });
  
        var GRIDbcout_41_aw_kontainer = MAIN_FRM.query("grid[pid=GRIDbcout_41_aw_kontainer]")[0];
        var STkontainer = GRIDbcout_41_aw_kontainer.getStore().getData();
        var DATA_kontainer = [];
        STkontainer.each(function (rec) {
          DATA_kontainer.push(rec.data);
        });
  
        var GRIDbcout_41_aw_kemasan = MAIN_FRM.query("grid[pid=GRIDbcout_41_aw_kemasan]")[0];
        var STkemasan = GRIDbcout_41_aw_kemasan.getStore().getData();
        var DATA_kemasan = [];
        STkemasan.each(function (rec) {
          DATA_kemasan.push(rec.data);
        });
  
        var GRIDbcout_41_aw_dokumen = MAIN_FRM.query("grid[pid=GRIDbcout_41_aw_dokumen]")[0];
        var STdokumen = GRIDbcout_41_aw_dokumen.getStore().getData();
        var DATA_dokumen = [];

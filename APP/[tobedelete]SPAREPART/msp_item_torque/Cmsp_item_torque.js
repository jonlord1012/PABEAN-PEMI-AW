Ext.define("TDK.SPAREPART.msp_item_torque.Cmsp_item_torque", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmsp_item_torque",
  init: function (view) {
    this.control({
      "msp_item_torque button[pid=btnew]": { click: this.btnew_click },
      "FRMmsp_item_torque button[pid=btnew_input]": { click: this.btnew_input_click },
      "FRMmsp_item_torque button[pid=btsimpan_draft]": { click: this.btsimpan_draft_click },
      "FRMmsp_item_torque button[pid=bthapus_draft]": { click: this.bthapus_draft_click },
      "FRMmsp_item_torque button[pid=btsearch]": { click: this.btsearch_click },
      "msp_item_torque grid[pid=GRIDmsp_item_torque]": { itemdblclick: this.GRIDmsp_item_torque_itemdblclick },
    });
    this.listen({
      store: {},
    });
  ...
  GRIDmsp_item_torque_load: function () {
    try {
      var me = this;
      var STGRIDnew = new Ext.data.Store({
        ...
        proxy: {
          ...
          url: vconfig.service_api + "msp_item_torque/msp_item_torques",
          reader: {
            type: "json",
            rootProperty: "Rows",
      ...
      var GRID = Ext.ComponentQuery.query("msp_item_torque GRIDmsp_item_torque grid[pid=GRIDmsp_item_torque]")[0];
      GRID.reconfigure(STGRIDnew);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  GRIDmsp_item_torque_itemdblclick: function (cmp, rec) {
    try {
      var me = this;
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMmsp_item_torque", "TDK.SPAREPART.msp_item_torque.FRMmsp_item_torque", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMmsp_item_torque form")[0];
  ...
  edit_header: function (FRM, vdt) {
    try {
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "msp_item_torque/msp_item_torque", params, "POST", me.var_global.jwt);
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMmsp_item_torque", "TDK.SPAREPART.msp_item_torque.FRMmsp_item_torque", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("TDK.EXIM.msp_item_torque.popup_search", {
        ...
        items: [
          {
            xtype: "grid",
            pid: "GRIDmsp_item_torque_popup_search",
  ...
  bthapus_draft_click: function (grid, rowIndex, colIndex) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("GRIDmsp_item_torque grid[pid=GRIDmsp_item_torque]")[0];
      var mainpanel = Ext.ComponentQuery.query("FRMmsp_item_torque")[0];
      var FRM = Ext.ComponentQuery.query("FRMmsp_item_torque form")[0];

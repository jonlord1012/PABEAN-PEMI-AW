Ext.define("TDK.SPAREPART.msp_item_cb.Cmsp_item_cb", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmsp_item_cb",
  init: function (view) {
    this.control({
      "msp_item_cb button[pid=btnew]": { click: this.btnew_click },
      "FRMmsp_item_cb button[pid=btnew_input]": { click: this.btnew_input_click },
      "FRMmsp_item_cb button[pid=btsimpan_draft]": { click: this.btsimpan_draft_click },
      "FRMmsp_item_cb button[pid=bthapus_draft]": { click: this.bthapus_draft_click },
      "FRMmsp_item_cb button[pid=btsearch]": { click: this.btsearch_click },
      "msp_item_cb grid[pid=GRIDmsp_item_cb]": { itemdblclick: this.GRIDmsp_item_cb_itemdblclick },
    });
    this.listen({
      store: {},
    });
    ...
    try {
      ...
      var STGRIDnew = new Ext.data.Store({
        ...
        proxy: {
          ...
          url: vconfig.service_api + "msp_item_cb/msp_item_cbs",
          reader: {
            type: "json",
            rootProperty: "Rows",
      ...
      var GRID = Ext.ComponentQuery.query("msp_item_cb GRIDmsp_item_cb grid[pid=GRIDmsp_item_cb]")[0];
      GRID.reconfigure(STGRIDnew);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  GRIDmsp_item_cb_itemdblclick: function (cmp, rec) {
    try {
      var me = this;
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMmsp_item_cb", "TDK.SPAREPART.msp_item_cb.FRMmsp_item_cb", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMmsp_item_cb form")[0];
  ...
  edit_header: function (FRM, vdt) {
    try {
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "msp_item_cb/msp_item_cb", params, "POST", me.var_global.jwt);
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMmsp_item_cb", "TDK.SPAREPART.msp_item_cb.FRMmsp_item_cb", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("TDK.EXIM.msp_item_cb.popup_search", {
        ...
        items: [
          {
            xtype: "grid",
            pid: "GRIDmsp_item_cb_popup_search",
  ...
  bthapus_draft_click: function (grid, rowIndex, colIndex) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("GRIDmsp_item_cb grid[pid=GRIDmsp_item_cb]")[0];
      var mainpanel = Ext.ComponentQuery.query("FRMmsp_item_cb")[0];
      var FRM = Ext.ComponentQuery.query("FRMmsp_item_cb form")[0];

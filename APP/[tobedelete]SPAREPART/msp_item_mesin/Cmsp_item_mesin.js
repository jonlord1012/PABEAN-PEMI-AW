Ext.define("TDK.SPAREPART.msp_item_mesin.Cmsp_item_mesin", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmsp_item_mesin",
  init: function (view) {
    this.control({
      "msp_item_mesin button[pid=btnew]": { click: this.btnew_click },
      "FRMmsp_item_mesin button[pid=btnew_input]": { click: this.btnew_input_click },
      "FRMmsp_item_mesin button[pid=btsimpan_draft]": { click: this.btsimpan_draft_click },
      "FRMmsp_item_mesin button[pid=bthapus_draft]": { click: this.bthapus_draft_click },
      "FRMmsp_item_mesin button[pid=btsearch]": { click: this.btsearch_click },
      "msp_item_mesin grid[pid=GRIDmsp_item_mesin]": { itemdblclick: this.GRIDmsp_item_mesin_itemdblclick },
    });
    this.listen({
      store: {},
    });
  ...
  GRIDmsp_item_mesin_load: function () {
    try {
      var me = this;
      var STGRIDnew = new Ext.data.Store({
        ...
        proxy: {
          ...
          url: vconfig.service_api + "msp_item_mesin/msp_item_mesins",
          reader: {
            type: "json",
            rootProperty: "Rows",
      ...
      var GRID = Ext.ComponentQuery.query("msp_item_mesin GRIDmsp_item_mesin grid[pid=GRIDmsp_item_mesin]")[0];
      GRID.reconfigure(STGRIDnew);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  GRIDmsp_item_mesin_itemdblclick: function (cmp, rec) {
    try {
      var me = this;
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMmsp_item_mesin", "TDK.SPAREPART.msp_item_mesin.FRMmsp_item_mesin", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMmsp_item_mesin form")[0];
  ...
  edit_header: function (FRM, vdt) {
    try {
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "msp_item_mesin/msp_item_mesin", params, "POST", me.var_global.jwt);
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMmsp_item_mesin", "TDK.SPAREPART.msp_item_mesin.FRMmsp_item_mesin", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("TDK.EXIM.msp_item_mesin.popup_search", {
        ...
        items: [
          {
            xtype: "grid",
            pid: "GRIDmsp_item_mesin_popup_search",
  ...
  bthapus_draft_click: function (grid, rowIndex, colIndex) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("GRIDmsp_item_mesin grid[pid=GRIDmsp_item_mesin]")[0];
      var mainpanel = Ext.ComponentQuery.query("FRMmsp_item_mesin")[0];
      var FRM = Ext.ComponentQuery.query("FRMmsp_item_mesin form")[0];

Ext.define("TDK.SPAREPART.msp_item_apli.Cmsp_item_apli", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmsp_item_apli",
  init: function (view) {
    this.control({
      "msp_item_apli button[pid=btnew]": { click: this.btnew_click },
      "FRMmsp_item_apli button[pid=btnew_input]": { click: this.btnew_input_click },
      "FRMmsp_item_apli button[pid=btsimpan_draft]": { click: this.btsimpan_draft_click },
      "FRMmsp_item_apli button[pid=bthapus_draft]": { click: this.bthapus_draft_click },
      "FRMmsp_item_apli button[pid=btsearch]": { click: this.btsearch_click },
      "msp_item_apli grid[pid=GRIDmsp_item_apli]": { itemdblclick: this.GRIDmsp_item_apli_itemdblclick },
    });
    this.listen({
      store: {},
    });
  ...
  GRIDmsp_item_apli_load: function () {
    try {
      var me = this;
      var STGRIDnew = new Ext.data.Store({
        ...
        proxy: {
          ...
          url: vconfig.service_api + "msp_item_apli/msp_item_aplis",
          reader: {
            type: "json",
            rootProperty: "Rows",
      ...
      var GRID = Ext.ComponentQuery.query("msp_item_apli GRIDmsp_item_apli grid[pid=GRIDmsp_item_apli]")[0];
      GRID.reconfigure(STGRIDnew);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  GRIDmsp_item_apli_itemdblclick: function (cmp, rec) {
    try {
      var me = this;
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMmsp_item_apli", "TDK.SPAREPART.msp_item_apli.FRMmsp_item_apli", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMmsp_item_apli form")[0];
  ...
  edit_header: function (FRM, vdt) {
    try {
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "msp_item_apli/msp_item_apli", params, "POST", me.var_global.jwt);
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMmsp_item_apli", "TDK.SPAREPART.msp_item_apli.FRMmsp_item_apli", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("TDK.EXIM.msp_item_apli.popup_search", {
        ...
        items: [
          {
            xtype: "grid",
            pid: "GRIDmsp_item_apli_popup_search",
  ...
  bthapus_draft_click: function (grid, rowIndex, colIndex) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("GRIDmsp_item_apli grid[pid=GRIDmsp_item_apli]")[0];
      var mainpanel = Ext.ComponentQuery.query("FRMmsp_item_apli")[0];
      var FRM = Ext.ComponentQuery.query("FRMmsp_item_apli form")[0];

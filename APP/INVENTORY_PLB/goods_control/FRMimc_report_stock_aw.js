...
Ext.define("NJC.INVENTORY_PLB.inv_material_control_aw.FRMimc_report_stock_aw", {
  extend: "Ext.window.Window",
  alias: "widget.FRMimc_report_stock_aw",
  reference: "FRMimc_report_stock_aw",
        ...
        proxy: {
          ...
          grouper : {
            groupFn: function(item){
                return 'Part Number' + item.get('BULAN') + ', Nomor Daftar: ' + item.get('NOMOR_DAFTAR'); ;
            }
          }
        },
        listeners: {
          beforeload: function (store, operation, eOpts) {
            try {
              var VIDCOMPANY = Ext.ComponentQuery.query("FRMimc_report_stock_aw combobox[name=CBO_COMPANY]")[0];
              var VPARTGROUP = Ext.ComponentQuery.query("FRMimc_report_stock_aw combobox[name=CBO_PARTGROUP]")[0];
              var VFROMDATE = Ext.ComponentQuery.query("FRMimc_report_stock_aw monthfield[name=tfromdate]")[0];
              var VTODATE = Ext.ComponentQuery.query("FRMimc_report_stock_aw monthfield[name=ttodate]")[0];
  ...
  dockedItems: [
    {
      ...
      items: [
        ...
        { xtype: "button", text: "Refresh", pid: "FRMimc_report_stock_aw_btrefresh", icon: vconfig.getstyle + "icon/update.ico", tooltip: "Refresh Data" },
        "-",
        {
          xtype: "combobox",
          name: "CBO_COMPANY",
        ...
        "-",
        {
          ...
          items: [
            ...
            {
              xtype: "button",
              pid: "FRMimc_report_stock_aw_btsearch",
              icon: vconfig.getstyle + "icon/search.ico",
              tooltip: "search",
              handler: "FRMimc_report_stock_aw_btsearch_click"
            },
          ],
        },
        "->",

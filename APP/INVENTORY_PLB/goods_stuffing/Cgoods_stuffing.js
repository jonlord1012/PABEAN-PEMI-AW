Ext.define("NJC.INVENTORY_PLB.goods_stuffing.Cgoods_stuffing", {
    ...
    init: function (view) {
        this.control({
            ...
            "goods_stuffing button[pid=btstuffing_integrasi]": { click: this.btstuffing_sync_click },
            "FRMgoods_stuffing_details grid[pid=GRIDFRM_good_picking_list]": { itemdblclick: this.GRIDFRM_good_picking_list_click },
    ...
    btstuffing_sync_click: function () {
        try {
            var me = this;
            ...
            var hasil = COMP.run.getservice(vconfig.service_api + "goods_stuffing/goods_stuffing", params, "POST", me.var_global.jwt);
    ...
    btsearch_picking_click: function (btn) {
        try {
            ...
            var GRIDPicking = Ext.create("NJC.INVENTORY_PLB.goods_stuffing.GRIDPicking", {
                                        ...
                                        Ext.each(rs, function (item) {
                                            ...
                                            if (!check_item) {
                                                GRIDFRM_good_picking_list.getStore().add({
                                                    ...
                                                    CUSTOMER_DRAFT: dtselect.CUSTOMER_DRAFT,
...
*/
    good_picking_list_load: function (xgrid, rowIndex) {
        try {
            ...
            //var custNo = Ext.ComponentQuery.query("FRMgoods_stuffing_details field[pid=CODE_CUSTOMER]")[0].value;
            //var invNo = Ext.ComponentQuery.query("FRMgoods_stuffing_details field[pid=INVOICE_NO]")[0].value;
            var GRID = Ext.ComponentQuery.query("goods_stuffing GRIDgoods_stuffing grid[pid=GRIDgoods_stuffing]")[0];
    ...
    GRIDpopup_list_source_itemdblclick: function (xgrid, rowIndex) {
        try {
            ...
            var GRID = Ext.ComponentQuery.query("FRMgoods_stuffing_details grid[pid=GRIDFRM_goods_picking_item]")[0];
            //var MAIN_dtval = FRM_MAIN.getValues(false, false, false, true);
            xgrid.getSelectionModel().select(rowIndex);
            var params = Ext.encode({
    ...
    GRIDFRM_good_picking_list_click: function (cmp, rec) {
        try {
            var me = this;
            var GRIDFRM_goods_picking_item = Ext.ComponentQuery.query("FRMgoods_stuffing_details grid[pid=GRIDFRM_goods_picking_item]")[0];
            GRIDFRM_goods_picking_item.getStore().load();
        } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
        }
    },
    FRM_goods_stuffing_btsave_click: function () {
        try {
            var me = this;
            ...
            */
            ...
            var labelInvoice = Ext.ComponentQuery.query("FRMgoods_stuffing_details field[pid=INVOICE_NO]")[0].value;
            var labelCustomer = Ext.ComponentQuery.query("FRMgoods_stuffing_details field[pid=CODE_CUSTOMER]")[0].value;
            GRIDDeliveryInstruction = Ext.ComponentQuery.query("FRMgoods_stuffing_details grid[pid=GRIDFRM_good_picking_list]")[0];

Ext.define("TDK.INVENTORY_AW.inv_wip_in_aw.Cinv_wip_in_aw", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cinv_wip_in_aw",
  init: function (view) {
    this.control({
      "inv_wip_in_aw button[pid=btreceiving_manual]": { click: this.btnew_click },
      "inv_wip_in_aw button[pid=btreceiving_integrasi]": { click: this.btreceiving_integrasi_click },
      "FRMinv_wip_in_aw button[pid=btsearch]": { click: this.btsearch_click },
      "FRMinv_wip_in_aw combobox[name=CBO_SOURCE]": { change: this.CBO_SOURCE_change },
      "FRMinv_wip_in_aw_pis button[pid=btrefresh]": { click: this.pisbtrefresh_click },
      "FRMinv_wip_in_aw_pis datefield[name=TANGGAL_RCV]": { change: this.pisbtrefresh_click },
    });
    this.listen({
      store: {},
    });
  ...
  GRIDinv_wip_in_aw_load: function (grid) {
    try {
      var me = this;
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMinv_wip_in_aw", "TDK.INVENTORY_AW.inv_wip_in_aw.FRMinv_wip_in_aw", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btreceiving_integrasi_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMinv_wip_in_aw_pis", "TDK.INVENTORY_AW.inv_wip_in_aw.FRMinv_wip_in_aw_pis", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btsearch_click: function (btn) {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMinv_wip_in_aw form")[0];
      ...
      var popup = Ext.define("TDK.INVENTORY_AW.inv_wip_in_aw.popup_list_source", {
  ...
  GRIDpopup_list_source_itemdblclick: function (cmp, rec) {
    try {
      ...
      var FRM_MAIN = Ext.ComponentQuery.query("FRMinv_wip_in_aw form")[0];
      var GRID = Ext.ComponentQuery.query("FRMinv_wip_in_aw grid[pid=GRIDFRMinv_wip_in_aw]")[0];
      var MAIN_dtval = FRM_MAIN.getValues(false, false, false, true);
      ...
      var hasil = COMP.run.getservice(vconfig.service_api + "inv_wip_in_aw/inv_wip_in_aw", params, "POST", me.var_global.jwt);
  ...
  FRMinv_wip_in_aw_btsave_click: function (btn) {
    try {
      var me = this;
      var popup = Ext.ComponentQuery.query("FRMinv_wip_in_aw")[0];
      var FRM_MAIN = Ext.ComponentQuery.query("FRMinv_wip_in_aw form")[0];
      var MAIN_dtval = FRM_MAIN.getValues(false, false, false, true);
      var GRID = Ext.ComponentQuery.query("FRMinv_wip_in_aw grid[pid=GRIDFRMinv_wip_in_aw]")[0];
      var Vgridvalidation = "";
  ...
  btdetail_rows_click: function (xgrid, rowIndex) {
    try {
      ...
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMinv_wip_in_aw", "TDK.INVENTORY_AW.inv_wip_in_aw.FRMinv_wip_in_aw", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMinv_wip_in_aw")[0];
  ...
  pisbtrefresh_click: function () {
    try {
      var GRID = Ext.ComponentQuery.query("FRMinv_wip_in_aw_pis grid[pid=GRIDinv_wip_in_aw_pis]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }

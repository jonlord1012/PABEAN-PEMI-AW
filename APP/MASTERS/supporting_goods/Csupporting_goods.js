Ext.define("NJC.MASTERS.supporting_goods.Csupporting_goods", {
  extend: "Ext.app.ViewController",
  alias: "controller.Csupporting_goods",
  init: function (view) {
    this.control({
      "supporting_goods button[pid=btnew]": { click: this.btnew_click },
      "FRMsupporting_goods button[pid=btnew_input]": { click: this.btnew_input_click },
      "FRMsupporting_goods button[pid=btsimpan_draft]": { click: this.btsimpan_draft_click },
      "FRMsupporting_goods button[pid=bthapus_draft]": { click: this.bthapus_draft_click },
      "FRMsupporting_goods button[pid=btsearch]": { click: this.btsearch_click },
      "supporting_goods grid[pid=GRIDsupporting_goods]": { itemdblclick: this.GRIDsupporting_goods_itemdblclick },
    });
    this.listen({
      store: {},
    });
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("NJC.EXIM.supporting_goods.popup_search", {
        ...
        items: [
          {
            xtype: "grid",
            pid: "GRIDsupporting_goods_popup_search",
  ...
  GRIDsupporting_goods_load: function () {
    try {
      var me = this;
      var STGRIDnew = new Ext.data.Store({
        ...
        proxy: {
          ...
          headers: { Authorization: "Bearer " + me.var_global.jwt },
          actionMethods: { read: "POST" },
          url: vconfig.service_api + "supporting_goods/supporting_goodss",
          extraParams: {
            method: "read_in",
          },
          reader: {
      ...
      var GRID = Ext.ComponentQuery.query("supporting_goods GRIDsupporting_goods grid[pid=GRIDsupporting_goods]")[0];
      GRID.reconfigure(STGRIDnew);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  GRIDsupporting_goods_itemdblclick: function (cmp, rec) {
    try {
      var me = this;
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMsupporting_goods", "NJC.MASTERS.supporting_goods.FRMsupporting_goods", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMsupporting_goods form")[0];
  ...
  edit_header: function (FRM, vdt) {
    try {
      ...
      var FRM = Ext.ComponentQuery.query("FRMsupporting_goods form")[0];
      var GRID = Ext.ComponentQuery.query("FRMsupporting_goods grid[pid=GRIDFRMsupporting_goods_parths]")[0];

      var hasil = COMP.run.getservice(vconfig.service_api + "supporting_goods/supporting_goods", params, "POST", me.var_global.jwt);
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMsupporting_goods", "NJC.MASTERS.supporting_goods.FRMsupporting_goods", mainpanel);
  ...
  bthapus_draft_click: function (grid, rowIndex, colIndex) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("GRIDsupporting_goods grid[pid=GRIDsupporting_goods]")[0];
      var mainpanel = Ext.ComponentQuery.query("FRMsupporting_goods")[0];
      var FRM = Ext.ComponentQuery.query("FRMsupporting_goods form")[0];
      ...
      Ext.MessageBox.confirm(
        "Konfirmasi",
        "Konfirmasi Hapus Data Supporting Goods!",
        function (button) {
          if (button === "yes") {
            var params = Ext.encode({
              method: "delete",
  ...
  exportTo: function (btn) {
    try {
      ...
      var cfg = Ext.merge(
        {
          title: "Supporting Goods",
          fileName: "Supporting Goods." + (btn.cfg.ext || btn.cfg.type),
          includeGroups: true,
          includeSummary: true,
        },
        btn.cfg

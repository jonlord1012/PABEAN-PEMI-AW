Ext.define("TDK.MASTERS.mst_item.Cmst_item", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmst_item",
  init: function (view) {
    this.control({
      "mst_item button[pid=btnew]": { click: this.btnew_click },
      "FRMmst_item button[pid=btnew_input]": { click: this.btnew_input_click },
      "FRMmst_item button[pid=btsimpan_draft]": { click: this.btsimpan_draft_click },
      "FRMmst_item button[pid=bthapus_draft]": { click: this.bthapus_draft_click },
      "FRMmst_item button[pid=btsearch]": { click: this.btsearch_click },
      "mst_item grid[pid=GRIDmst_item]": { itemdblclick: this.GRIDmst_item_itemdblclick },
    });
    this.listen({
      store: {},
    });
  ...
  popup_search(btn) {
    try {
      ...
      var popup = Ext.define("TDK.EXIM.mst_item.popup_search", {
  ...
  GRIDmst_item_load: function () {
    try {
      ...
      var GRID = Ext.ComponentQuery.query("mst_item GRIDmst_item grid[pid=GRIDmst_item]")[0];
      GRID.reconfigure(STGRIDnew);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  GRIDmst_item_itemdblclick: function (cmp, rec) {
    try {
      var me = this;
      var vdt = rec.data;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var load_edit = COMP.run.getmodulepopup("FRMmst_item", "TDK.MASTERS.mst_item.FRMmst_item", mainpanel);
      if (load_edit) {
        var FRM = Ext.ComponentQuery.query("FRMmst_item form")[0];
  ...
  edit_header: function (FRM, vdt) {
    try {
      ...
      var GRID = Ext.ComponentQuery.query("FRMmst_item grid[pid=GRIDFRMmst_item_parths]")[0];

      var hasil = COMP.run.getservice(vconfig.service_api + "mst_item/mst_item", params, "POST", me.var_global.jwt);
  ...
  btnew_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("FRMmst_item", "TDK.MASTERS.mst_item.FRMmst_item", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  ...
  bthapus_draft_click: function (grid, rowIndex, colIndex) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("GRIDmst_item grid[pid=GRIDmst_item]")[0];
      var mainpanel = Ext.ComponentQuery.query("FRMmst_item")[0];
      var FRM = Ext.ComponentQuery.query("FRMmst_item form")[0];
      ...
      var cfg = Ext.merge(
        {
          ...
          fileName: "Master Item/Part." + (btn.cfg.ext || btn.cfg.type),
          includeGroups: true,
          includeSummary: true,
        },
        btn.cfg

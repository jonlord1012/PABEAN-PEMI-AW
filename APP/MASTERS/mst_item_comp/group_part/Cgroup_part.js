Ext.define("NJC.MASTERS.mst_item_comp.group_part.Cgroup_part", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cgroup_part",
  init: function (view) {
    this.control({
      "group_part FRMgroup_part grid[pid=GRIDgroup_part]": { itemdblclick: this.GRIDgroup_part_itemdblclick },
      "group_part button[pid=btnew_input]": { click: this.btnew_input_click },
      "group_part button[pid=btsave]": { click: this.btsave_click },
      "group_part button[pid=btdelete]": { click: this.btdelete_click },
    });
    this.var_global = {
  ...
  btnew_input_click: function () {
    try {
      var FRM = Ext.ComponentQuery.query("FRMgroup_part form")[0];
  ...
  GRIDgroup_part_itemdblclick: function (cmp, rec) {
    try {
      var FRM = Ext.ComponentQuery.query("FRMgroup_part form")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  btsave_click: function () {
    try {
      var me = this;
      var FRM = Ext.ComponentQuery.query("FRMgroup_part form")[0];
      var GRID = Ext.ComponentQuery.query("FRMgroup_part grid[pid=GRIDgroup_part]")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  exportTo: function (btn) {
    try {
      ...
      var cfg = Ext.merge(
        {
          title: "Master Group Part",
          fileName: "Master Group Part." + (btn.cfg.ext || btn.cfg.type),
          includeGroups: true,
          includeSummary: true,
        },
        btn.cfg

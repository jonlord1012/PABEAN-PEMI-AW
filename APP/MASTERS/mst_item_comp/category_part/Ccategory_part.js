Ext.define("NJC.MASTERS.mst_item_comp.category_part.Ccategory_part", {
  extend: "Ext.app.ViewController",
  alias: "controller.Ccategory_part",
  init: function (view) {
    this.control({
      "category_part FRMcategory_part grid[pid=GRIDcategory_part]": { itemdblclick: this.GRIDcategory_part_itemdblclick },
      "category_part button[pid=btnew_input]": { click: this.btnew_input_click },
      "category_part button[pid=btsave]": { click: this.btsave_click },
      "category_part button[pid=btdelete]": { click: this.btdelete_click },
    });
    this.var_global = {
  ...
  btnew_input_click: function () {
    try {
      var FRM = Ext.ComponentQuery.query("FRMcategory_part form")[0];
  ...
  GRIDcategory_part_itemdblclick: function (cmp, rec) {
    try {
      var FRM = Ext.ComponentQuery.query("FRMcategory_part form")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  btsave_click: function () {
    try {
      var me = this;
      var FRM = Ext.ComponentQuery.query("FRMcategory_part form")[0];
      var GRID = Ext.ComponentQuery.query("FRMcategory_part grid[pid=GRIDcategory_part]")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  exportTo: function (btn) {
    try {
      ...
      var cfg = Ext.merge(
        {
          title: "Master Category Part",
          fileName: "Master Category Part." + (btn.cfg.ext || btn.cfg.type),
          includeGroups: true,
          includeSummary: true,
        },
        btn.cfg

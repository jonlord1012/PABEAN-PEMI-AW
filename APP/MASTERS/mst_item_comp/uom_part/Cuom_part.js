Ext.define("NJC.MASTERS.mst_item_comp.uom_part.Cuom_part", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cuom_part",
  init: function (view) {
    this.control({
      "uom_part FRMuom_part grid[pid=GRIDuom_part]": { itemdblclick: this.GRIDuom_part_itemdblclick },
      "uom_part button[pid=btnew_input]": { click: this.btnew_input_click },
      "uom_part button[pid=btsave]": { click: this.btsave_click },
      "uom_part button[pid=btdelete]": { click: this.btdelete_click },
    });
    this.var_global = {
  ...
  btnew_input_click: function () {
    try {
      var FRM = Ext.ComponentQuery.query("FRMuom_part form")[0];
  ...
  GRIDuom_part_itemdblclick: function (cmp, rec) {
    try {
      var FRM = Ext.ComponentQuery.query("FRMuom_part form")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  btsave_click: function () {
    try {
      var me = this;
      var FRM = Ext.ComponentQuery.query("FRMuom_part form")[0];
      var GRID = Ext.ComponentQuery.query("FRMuom_part grid[pid=GRIDuom_part]")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  exportTo: function (btn) {
    try {
      ...
      var cfg = Ext.merge(
        {
          title: "Master UOM Part",
          fileName: "Master UOM Part." + (btn.cfg.ext || btn.cfg.type),
          includeGroups: true,
          includeSummary: true,
        },
        btn.cfg

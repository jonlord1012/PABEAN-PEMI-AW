Ext.define("NJC.MASTERS.mst_item_comp.base_part.Cbase_part", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cbase_part",
  init: function (view) {
    this.control({
      "base_part FRMbase_part grid[pid=GRIDbase_part]": { itemdblclick: this.GRIDbase_part_itemdblclick },
      "base_part button[pid=btnew_input]": { click: this.btnew_input_click },
      "base_part button[pid=btsave]": { click: this.btsave_click },
      "base_part button[pid=btdelete]": { click: this.btdelete_click },
    });
    this.var_global = {
  ...
  btnew_input_click: function () {
    try {
      var FRM = Ext.ComponentQuery.query("FRMbase_part form")[0];
  ...
  GRIDbase_part_itemdblclick: function (cmp, rec) {
    try {
      var FRM = Ext.ComponentQuery.query("FRMbase_part form")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  btsave_click: function () {
    try {
      var me = this;
      var FRM = Ext.ComponentQuery.query("FRMbase_part form")[0];
      var GRID = Ext.ComponentQuery.query("FRMbase_part grid[pid=GRIDbase_part]")[0];
      var MAIN_dtval = FRM.getValues(false, false, false, true);
  ...
  exportTo: function (btn) {
    try {
      ...
      var cfg = Ext.merge(
        {
          title: "Master Base Part",
          fileName: "Master Base Part." + (btn.cfg.ext || btn.cfg.type),
          includeGroups: true,
          includeSummary: true,
        },
        btn.cfg

Ext.define("TDK.SYNCHRONIZE.sync_doc_lp.Csync_doc_lp", {
  extend: "Ext.app.ViewController",
  alias: "controller.Csync_doc_lp",
  init: function (view) {
    this.control({
      "sync_doc_lp button[pid=btrefresh]": { click: this.btrefresh_click },
      "sync_doc_lp combobox[name=CBO_FILTERKEY]": { select: this.btrefresh_click },
      "sync_doc_lp button[pid=btprocess_sync]": { click: this.btprocess_sync_click },
      "sync_doc_lp button[pid=btprocess_syncaju]": { click: this.btprocess_syncaju_click },
      "sync_doc_lp button[pid=btmapping_supplier]": { click: this.btmapping_supplier_click },
      "sync_doc_lp button[pid=btmapping_itempart]": { click: this.btmapping_itempart_click },
      "sync_doc_lp button[pid=btdokumen_draft]": { click: this.btdokumen_draft_click },

      "lp_popupmapping_supplier button[pid=btautomapping]": { click: this.btautomapping_click },
      "lp_popupmapping_itempart button[pid=btautomapping_itempart]": { click: this.btautomapping_itempart_click },

      "lp_FRMdraft_dokumen button[pid=btlp_savedokumendraft]": { click: this.btlp_savedokumendraft_click },
      "lp_FRMdraft_dokumen button[pid=btsearch]": { click: this.btsearch_click },
      "lp_FRMdraft_dokumen button[pid=btsearch_invoice]": { click: this.btsearch_invoice_click },
    });
    this.listen({
      store: {},
    });
  ...
  btmapping_supplier_click: function () {
    try {
      ...
      COMP.run.getmodulepopup("lp_popupmapping_supplier", "TDK.SYNCHRONIZE.sync_doc_lp.lp_popupmapping_supplier", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btmapping_itempart_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("lp_popupmapping_itempart", "TDK.SYNCHRONIZE.sync_doc_lp.lp_popupmapping_itempart", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btdokumen_draft_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("lp_FRMdraft_dokumen", "TDK.SYNCHRONIZE.sync_doc_lp.lp_FRMdraft_dokumen", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btpilihsupplier_mapping: function (grid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.SYNCHRONIZE.sync_doc_lp.popup_selectvendor", {
  ...
  select_manualsupplier: function (cmp, rec) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("lp_popupmapping_supplier grid[pid=GRIDmappingdata_supplier]")[0];
  ...
  btpilihpart_mapping: function (grid, rowIndex) {
    try {
      ...
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.SYNCHRONIZE.sync_doc_lp.popup_selectpart", {
        extend: "Ext.window.Window",
  ...
  select_manual_itempart: function (cmp, rec) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("lp_popupmapping_itempart grid[pid=GRIDmappingdata_itempart]")[0];
  ...
  lp_show_tracingdocument_rowclick: function (grid, rowIndex) {
    try {
      grid.getSelectionModel().select(rowIndex);
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("lp_tracing_document", "TDK.SYNCHRONIZE.sync_doc_lp.lp_tracing_document", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  lp_accordion_list_click: function (cmp, dt) {
    try {
      var vdt = dt.data;
      if (vdt.allowclick !== true) return;
      var vmodulecontrol = vdt.controlname;
      var id = vmodulecontrol;
      var cls = "TDK.SYNCHRONIZE.sync_doc_lp.tracing." + vmodulecontrol;
      var tabs = Ext.ComponentQuery.query("lp_tracing_document tabpanel[pid=lp_tabpanel_tracing]")[0];
  ...
  btsearch_click: function (btn) {
    try {
      ...
      var popup = Ext.define("TDK.SYNCHRONIZE.sync_doc_lp.popup_btsearch", {
        extend: "Ext.window.Window",
        ...
        items: [
          {
            ...
            listeners: {
              itemdblclick: function (grid, rec) {
                switch (btn.vdata.modulename) {
                  case "supplier":
                    var FRM = Ext.ComponentQuery.query("lp_FRMdraft_dokumen form")[0];
  ...
  btlp_savedokumendraft_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var FRM = Ext.ComponentQuery.query("lp_FRMdraft_dokumen form")[0];
  ...
  btsearch_invoice_click: function () {
    try {
      var me = this;
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      var popup = Ext.define("TDK.SYNCHRONIZE.sync_doc_lp.popup_selectinvoice", {
        extend: "Ext.window.Window",
        ...
        items: [
          {
            ...
            tbar: [
              //
              "-",
              { xtype: "button", pid: "btlp_select_invoice_draft", text: "Pilih Invoice", icon: vconfig.getstyle + "icon/check.png", tooltip: "Pilih Invoice" },
            ],
          },
        ],
      });

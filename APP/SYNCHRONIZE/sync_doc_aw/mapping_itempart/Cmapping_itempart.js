Ext.define("NJC.SYNCHRONIZE.sync_doc_aw.mapping_itempart.Cmapping_itempart", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmapping_itempart",
  init: function (view) {
    this.control({
      "aw_mapping_itempart button[pid=btautomapping]": { click: this.btautomapping_click },
    });
    this.listen({
      store: {},
    });
  ...
  btpilihpart_mapping: function (grid, rowIndex) {
    try {
      ...
      var popup = Ext.define("NJC.SYNCHRONIZE.sync_doc_aw.mapping_itempart.popup_selectpart", {
        extend: "Ext.window.Window",
        alias: "widget.popup_selectpart",
        reference: "popup_selectpart",
        title: "Mapping Manual Item/Part Material " + vdt.PART_CODE,
        modal: true,
        closeAction: "destroy",
        centered: true,
        autoScroll: true,
  ...
  select_manual_itempart: function (cmp, rec) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("aw_mapping_itempart FRMmapping_itempart grid[pid=GRIDmappingdata_itempart]")[0];
      var popup = Ext.ComponentQuery.query("popup_selectpart")[0];
      var vdt = GRID.getSelectionModel().getSelection()[0].data;
      var params = Ext.encode({
        method: "sync_manualmapping_itempart",
        module: "aw",
        PART_CODE: vdt.PART_CODE,

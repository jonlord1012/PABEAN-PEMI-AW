Ext.define("NJC.SYNCHRONIZE.sync_doc_aw.mapping_supplier.Cmapping_supplier", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmapping_supplier",
  init: function (view) {
    this.control({
      "aw_mapping_supplier button[pid=btautomapping]": { click: this.btautomapping_click },
    });
    this.listen({
      store: {},
    });
  ...
  btpilihsupplier_mapping: function (grid, rowIndex) {
    try {
      ...
      var popup = Ext.define("NJC.SYNCHRONIZE.sync_doc_aw.mapping_supplier.popup_selectvendor", {
        ...
        items: [
          {
            ...
            columns: [
              ...
              { header: "AW CODE", dataIndex: "KODE_AW", sortable: true, width: 80, filter: { xtype: "textfield" } },
  ...
  select_manualsupplier: function (cmp, rec) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("aw_mapping_supplier FRMmapping_supplier grid[pid=GRIDmappingdata_supplier]")[0];

Ext.define("TDK.SYNCHRONIZE.sync_doc_coo.mapping_itempart.Cmapping_itempart", {
  extend: "Ext.app.ViewController",
  alias: "controller.Ccoomapping_itempart",
  init: function (view) {
    this.control({
      "coo_mapping_itempart button[pid=btautomapping]": { click: this.btautomapping_click },
    });
    this.listen({
      store: {},
    });
  ...
  btpilihpart_mapping: function (grid, rowIndex) {
    try {
      ...
      var popup = Ext.define("TDK.SYNCHRONIZE.sync_doc_coo.mapping_itempart.popup_selectpart", {
  ...
  select_manual_itempart: function (cmp, rec) {
    try {
      var me = this;
      var GRID = Ext.ComponentQuery.query("cooFRMmapping_itempart grid[pid=GRIDmappingdata_itempart]")[0];
  ...
  btautomapping_click: function () {
    try {
      ...
      var GRID = Ext.ComponentQuery.query("coomapping_itempart cooFRMmapping_itempart grid[pid=GRIDmappingdata_itempart]")[0];
      var hasil = COMP.run.getservice(vconfig.service_api + "sync_doc/sync_doc", params, "POST", me.var_global.jwt);
      hasil.then(function (content) {
        var val = Ext.decode(content, true);
        if (val.success == "true") {

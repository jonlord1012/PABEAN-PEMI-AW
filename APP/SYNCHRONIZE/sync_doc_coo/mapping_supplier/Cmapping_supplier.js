Ext.define("TDK.SYNCHRONIZE.sync_doc_coo.mapping_supplier.Cmapping_supplier", {
  extend: "Ext.app.ViewController",
  alias: "controller.Ccoo_mapping_supplier",
  init: function (view) {
    this.control({
      "coo_mapping_supplier button[pid=btautomapping]": { click: this.btautomapping_click },
    });
    this.listen({
      store: {},
    });
  ...
  btpilihsupplier_mapping: function (grid, rowIndex) {
    try {
      ...
      var popup = Ext.define("TDK.SYNCHRONIZE.sync_doc_coo.mapping_supplier.popup_selectvendor", {
  ...
  btautomapping_click: function () {
    try {
      ...
      var GRID = Ext.ComponentQuery.query("coo_mapping_supplier cooFRMmapping_supplier grid[pid=GRIDmappingdata_supplier]")[0];
      var hasil = COMP.run.getservice(vconfig.service_api + "sync_doc/sync_doc", params, "POST", me.var_global.jwt);
      hasil.then(function (content) {
        var val = Ext.decode(content, true);
        if (val.success == "true") {

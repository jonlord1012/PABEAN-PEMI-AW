Ext.define("TDK.SYNCHRONIZE.sync_doc_sa_aw.Csync_doc_sa_aw", {
  extend: "Ext.app.ViewController",
  alias: "controller.Csync_doc_sa_aw",
  init: function (view) {
    this.control({
      "sync_doc_sa_aw button[pid=btrefresh]": { click: this.btrefresh_click },
      "sync_doc_sa_aw combobox[name=CBO_FILTERKEY]": { change: this.btrefresh_click },
      "sync_doc_sa_aw button[pid=btproses_draft]": { click: this.btproses_draft_click },
    });
    this.listen({
      store: {},
    });
  ...
  renderpage: function () {
    try {
      console.log("renderer controller sync_doc_sa_aw");
    } catch (ex) {
  ...
  btrefresh_click: function () {
    try {
      var GRIDsync_doc_sa_aw = Ext.ComponentQuery.query("GRIDsync_doc_sa_aw grid[pid=GRIDsync_doc_sa_aw]")[0];
      GRIDsync_doc_sa_aw.getStore().load();
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btproses_draft_click: function () {
    try {
      var GRIDsync_doc_sa_aw = Ext.ComponentQuery.query("GRIDsync_doc_sa_aw grid[pid=GRIDsync_doc_sa_aw]")[0];
      var GRIDsync_doc_sa_aw = GRIDsync_doc_sa_aw.getSelectionModel();
      var mainpanel = Ext.ComponentQuery.query("mainpage")[0];
      COMP.run.getmodulepopup("FRMsync_doc_sa_aw_draft", "TDK.SYNCHRONIZE.sync_doc_sa_aw.FRMsync_doc_sa_aw_draft", mainpanel);
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btsearch_sa_list_click: function () {
    try {
      var me = this;
      var FRMsync_doc_sa_aw_draft = Ext.ComponentQuery.query("FRMsync_doc_sa_aw_draft form")[0];
      var vdt = FRMsync_doc_sa_aw_draft.getValues(false, false, false, true);
      ...
      var popup = new Ext.define("TDK.SYNCHRONIZE.sync_doc_sa_aw.popup_dokumen_sa_aw", {
        extend: "Ext.window.Window",
        alias: "widget.popup_dokumen_sa_aw",
        reference: "popup_dokumen_sa_aw",
        ...
        items: [
          {
            ...
            listeners: {
              itemdblclick: function (xgrid, rec) {
                var vdt = rec.data;
                var FRMsync_doc_sa_aw_draft = Ext.ComponentQuery.query("FRMsync_doc_sa_aw_draft form")[0];
                var dtform = FRMsync_doc_sa_aw_draft.getValues(false, false, false, true);
                var popup = Ext.ComponentQuery.query("popup_dokumen_sa_aw")[0];
                var GRIDinvoice_header = Ext.ComponentQuery.query("FRMsync_doc_sa_aw_draft grid[pid=GRIDinvoice_header]")[0];
                var GRIDinvoice_item = Ext.ComponentQuery.query("FRMsync_doc_sa_aw_draft grid[pid=GRIDinvoice_item]")[0];
                FRMsync_doc_sa_aw_draft.getForm().setValues({
                  SANO: vdt.SANO,
                  SADATE: vdt.SADATE,
                  ETDPEMI: vdt.ETDPEMI,
      ...
      COMP.run.getmodulepopup("popup_dokumen_sa_aw", popup, this.getView());
    } catch (ex) {
      COMP.TipToast.toast("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },

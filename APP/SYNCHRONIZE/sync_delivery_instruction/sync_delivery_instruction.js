Ext.define("NJC.SYNCHRONIZE.sync_delivery_instruction.sync_delivery_instruction", {
  extend: "Ext.form.Panel",
  alias: "widget.sync_delivery_instruction",
  reference: "sync_delivery_instruction",
  config: {},
  requires: ["NJC.SYNCHRONIZE.sync_delivery_instruction.GRIDsync_delivery_instruction", "NJC.SYNCHRONIZE.sync_delivery_instruction.Csync_delivery_instruction"],
  constructor: function (config) {
    return this.callParent(arguments);
  },
  //untuk include controller
  controller: "Csync_delivery_instruction",
  initComponent: function () {
    Ext.apply(this, {
      xtype: "panel",
      pid: "panelsync_delivery_instruction",
      layout: "card",
      frame: false,
      border: false,
      items: [{ xtype: "GRIDsync_delivery_instruction" }],
      dockedItems: [
        {
          xtype: "toolbar",
          dock: "top",
          items: [
            "-",
            {
              xtype: "combobox",
              name: "CBO_FILTERKEY",
              fieldLabel: "Filter Data Key",
              labelWidth: 80,
              width: 350,
              displayField: "DEFNAME",
              valueField: "DEFCODE",
              fieldCls: "fieldinput",
              allowBlank: false,
              queryMode: "local",
              forceSelection: true,
              typeAhead: true,
              minChars: 0,
              anyMatch: true,
              value: "0",
              store: new Ext.data.Store({
                data: [
                  { DEFCODE: "0", DEFNAME: "All Data" },
                  { DEFCODE: "1", DEFNAME: "Outstanding Delivery" },
                  // { DEFCODE: "1", DEFNAME: "Belum Mapping Supplier" },
                  // { DEFCODE: "2", DEFNAME: "Belum Mapping Item/Part" },
                  // { DEFCODE: "3", DEFNAME: "Belum Ada Dokumen BC" },
                  // { DEFCODE: "4", DEFNAME: "Belum Proses Receiving Dokumen" },
                  // { DEFCODE: "5", DEFNAME: "Dokumen BC 23" },
                  // { DEFCODE: "6", DEFNAME: "Dokumen BC 23 Kurir" },
                  // { DEFCODE: "7", DEFNAME: "Dokumen BC 20 Kurir" },
                  // { DEFCODE: "8", DEFNAME: "Dokumen BC 40" },
                  // { DEFCODE: "9", DEFNAME: "Dokumen BC 27" },
                  // { DEFCODE: "10", DEFNAME: "Dokumen BC dalam proses Draft" },
                ],
                fields: ["DEFCODE", "DEFNAME"],
              }),
            },
            "-",
            { xtype: "button", pid: "btrefresh", icon: vconfig.getstyle + "icon/update.ico", tooltip: "Refresh Data" },
            "-",
            { xtype: "button", pid: "btdelivery_sync", html: "Sync Delivery Instruction", icon: vconfig.getstyle + "icon/table_refresh.png", tooltip: "Sinkronisasi Delivery Instruction" },
            "-",
            "-",
            { xtype: "button", pid: "btcreate_draft", html: "Create Draft BC", icon: vconfig.getstyle + "icon/create_bc.png", tooltip: "Pembuatan Draft Dokumen BC" },
            "->",
            {
              xtype: "button",
              text: "Export to",
              menu: {
                defaults: {
                  handler: "exportTo",
                },
                items: [
                  {
                    text: "Excel xlsx",
                    icon: vconfig.getstyle + "icon/exceldownload.png",
                    cfg: {
                      type: "excel07",
                      ext: "xlsx",
                    },
                    title: " AW ",
                    format: ".xlsx"
                  },
                ],
              },
            },
            // "-",
            // { xtype: "component", html: "Mapping" },
            // { xtype: "button", pid: "btmapping_supplier", icon: vconfig.getstyle + "icon/show_complete.png", tooltip: "Mapping Supplier" },
            // { xtype: "button", pid: "btmapping_itempart", icon: vconfig.getstyle + "icon/show_all.png", tooltip: "Mapping Item Part/Material" },
            // "-",
            // { xtype: "component", html: "Draft" },
            // { xtype: "button", pid: "btdokumen_draft", icon: vconfig.getstyle + "icon/paperclip.ico", tooltip: "Pembuatan Draft Dokumen" },
            // "-",
            // "->",
            // { xtype: "button", text: "Log", pid: "btlog_aw", icon: vconfig.getstyle + "icon/Book%201.ico", tooltip: "Log Dokumen AW" },
          ],
          // other options....
        },
      ],
    });

    this.callParent(arguments);
  },
});
